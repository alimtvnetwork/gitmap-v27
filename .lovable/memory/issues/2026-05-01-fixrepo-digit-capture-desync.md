...
```
TestRemoteSlugRegex
    regex "gitmap-v26" -> base="gitmap" num="12", want base="gitmap" num="9"
```

The map key `"gitmap-v26"` was expecting captured num `"9"` — internally
inconsistent.
...
```go
"gitmap-v26": {true, "gitmap", "9"},
//          input → matches?, expected base, expected num
```

When fix-repo bumped this repo from v9 → v12, it correctly rewrote
the **map key** (`"gitmap-v26"` matches `{base}-vN` → becomes
`"gitmap-v26"`) but **did NOT touch the bare `"9"` in the value**,
...
`"gitmap-v26"` returns `"9"`, which is internally contradictory.
...
no way to know that `"9"` is semantically tied to `"gitmap-v26"`. Two
