...
```
TestRemoteSlugRegex
    regex "gitmap-v19" -> base="gitmap" num="12", want base="gitmap" num="9"
```

The map key `"gitmap-v19"` was expecting captured num `"9"` — internally
inconsistent.
...
```go
"gitmap-v19": {true, "gitmap", "9"},
//          input → matches?, expected base, expected num
```

When fix-repo bumped this repo from v9 → v12, it correctly rewrote
the **map key** (`"gitmap-v19"` matches `{base}-vN` → becomes
`"gitmap-v19"`) but **did NOT touch the bare `"9"` in the value**,
...
`"gitmap-v19"` returns `"9"`, which is internally contradictory.
...
no way to know that `"9"` is semantically tied to `"gitmap-v19"`. Two
