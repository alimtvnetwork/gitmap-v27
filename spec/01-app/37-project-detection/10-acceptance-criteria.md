...
1. `gitmap-v26 go-repos` returns Go projects from DB.
2. `gitmap-v26 node-repos` returns Node.js projects from DB.
3. `gitmap-v26 react-repos` returns React projects from DB.
4. `gitmap-v26 cpp-repos` returns C++ projects from DB.
5. `gitmap-v26 csharp-repos` returns C# projects from DB.
...
1. `gitmap-v26 projects` — unified command listing all types grouped.
2. `--type` flag for filtering: `gitmap-v26 projects --type go,react`.
...
7. Dry-run mode: `gitmap-v26 scan --detect-only`.
