...
Every gitmap-v26 command supports a `--help` flag that prints detailed
...
```
gitmap-v26/helptext/<command-name>.md
```

Example files:

```
gitmap-v26/helptext/scan.md
gitmap-v26/helptext/clone.md
gitmap-v26/helptext/cd.md
gitmap-v26/helptext/go-repos.md
gitmap-v26/helptext/release.md
...
```
...
- Run `gitmap-v26 scan` first to populate the database (link to scan.md)
...
A new package `gitmap-v26/helptext` bundles all `.md` files:
...
When a user runs `gitmap-v26 <command> --help`:
...
import "github.com/alimtvnetwork/gitmap-v26/gitmap-v26/helptext"
...
For full details, each entry links to `gitmap-v26/helptext/<command>.md`.
...
1. Create `gitmap-v26/helptext/` directory with all 41 `.md` files
2. Create `gitmap-v26/helptext/print.go` with `go:embed` and `Print` function
3. Create `gitmap-v26/cmd/helpcheck.go` with `checkHelp` function
...
- [ ] `gitmap-v26 scan --help` prints scan help with examples and exits 0
- [ ] `gitmap-v26 cd --help` prints cd help including prerequisites
- [ ] `gitmap-v26 go-repos -h` prints project detection help
...
- [ ] `gitmap-v26 help` continues to print the existing summary usage
