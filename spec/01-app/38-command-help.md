...
Every gitmap-v23 command supports a `--help` flag that prints detailed
...
```
gitmap-v23/helptext/<command-name>.md
```

Example files:

```
gitmap-v23/helptext/scan.md
gitmap-v23/helptext/clone.md
gitmap-v23/helptext/cd.md
gitmap-v23/helptext/go-repos.md
gitmap-v23/helptext/release.md
...
```
...
- Run `gitmap-v23 scan` first to populate the database (link to scan.md)
...
A new package `gitmap-v23/helptext` bundles all `.md` files:
...
When a user runs `gitmap-v23 <command> --help`:
...
import "github.com/alimtvnetwork/gitmap-v23/gitmap-v23/helptext"
...
For full details, each entry links to `gitmap-v23/helptext/<command>.md`.
...
1. Create `gitmap-v23/helptext/` directory with all 41 `.md` files
2. Create `gitmap-v23/helptext/print.go` with `go:embed` and `Print` function
3. Create `gitmap-v23/cmd/helpcheck.go` with `checkHelp` function
...
- [ ] `gitmap-v23 scan --help` prints scan help with examples and exits 0
- [ ] `gitmap-v23 cd --help` prints cd help including prerequisites
- [ ] `gitmap-v23 go-repos -h` prints project detection help
...
- [ ] `gitmap-v23 help` continues to print the existing summary usage
