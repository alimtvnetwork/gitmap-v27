...
When `gitmap-v23 release` runs in a repository that contains a Go project
...
1. `gitmap-v23 release v1.0.0` in a Go repo produces 6 binaries + checksums.
2. `gitmap-v23 release --dry-run` lists all planned binaries without building.
3. `gitmap-v23 release --no-assets` skips binary compilation entirely.
4. `gitmap-v23 release --targets windows/amd64,linux/amd64` builds only 2 binaries.
5. `gitmap-v23 release --compress` produces `.zip`/`.tar.gz` archives.
