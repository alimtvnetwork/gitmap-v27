...
gitmap-v26 supports Windows, Linux, and macOS with feature parity across
...
The `gitmap-v26/Makefile` provides a standard interface wrapping `run.sh`:
...
3. Run `gitmap-v26 release` with `--compress --checksums`
...
- Binary names: `gitmap-v26` on Unix, `gitmap.exe` on Windows
