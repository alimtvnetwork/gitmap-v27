...
The `gitmap-v26 ls` command should provide transparent feedback about
...
```
  ...
  No repos tracked. Run 'gitmap-v26 scan' first.
```
...
- **Given** `gitmap-v26 ls --verbose` with repos, **then** DB path is printed
  above the repo table.
- **Given** `gitmap-v26 ls` with zero repos, **then** DB path is printed
