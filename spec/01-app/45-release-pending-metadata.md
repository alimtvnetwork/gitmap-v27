...
Enhances `gitmap-v26 release-pending` (alias `rp`) to discover unreleased
...
1. `gitmap-v26 rp` with only `.gitmap/release/v1.0.0.json` (no branch/tag) creates
   `release/v1.0.0` branch, `v1.0.0` tag, and pushes both.
2. `gitmap-v26 rp` with both a pending branch AND a metadata-only version
   processes both without duplicates.
3. `gitmap-v26 rp --dry-run` shows metadata-based releases without executing.
4. Missing/invalid commit SHA in metadata prints a warning and continues.
5. `gitmap-v26 rp` with no pending branches and no metadata files prints
