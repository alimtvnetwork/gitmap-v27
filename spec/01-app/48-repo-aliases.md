...
repository. Once aliased, any `gitmap-v26` command can target the repo by
...
1. **Short names** — replace long slugs or paths with concise aliases
   (e.g., `api`, `web`, `infra`).
2. **Run from anywhere** — execute any gitmap-v26 command against a repo
...
```
gitmap-v26 alias set <alias> <slug-or-path>
gitmap-v26 alias remove <alias>
gitmap-v26 alias list
gitmap-v26 alias show <alias>
gitmap-v26 alias suggest [--apply]
```
...
Any gitmap-v26 command that operates on a repository accepts `-A <alias>`
...
gitmap-v26 exec -A web -- npm test

# Navigate to the aliased repo
gitmap-v26 cd -A infra

# Check status of an aliased repo
gitmap-v26 status -A api

# Watch an aliased repo
gitmap-v26 watch -A web
```
...
When `gitmap-v26 scan` or `gitmap-v26 rescan` discovers new repositories, the
system suggests aliases based on:

1. **Repo name** — last segment of the clone URL (e.g., `gitmap-v26`).
...
```bash
gitmap-v26 scan --apply-aliases    # Accept all suggestions automatically
gitmap-v26 alias suggest --apply   # Suggest for existing unaliased repos
```
...
```bash
gitmap-v26 completion --list-aliases
...
- Alias name conflicts with a gitmap-v26 subcommand → `alias cannot shadow command: <name>` (exit 1).

---

## Acceptance Criteria

1. `gitmap-v26 alias set api github/user/api-gateway` creates an alias.
2. `gitmap-v26 alias list` displays all aliases with their target slugs.
3. `gitmap-v26 alias show api` shows the linked repo details.
4. `gitmap-v26 alias remove api` deletes the alias.
5. `gitmap-v26 pull -A api` pulls the repo from any working directory.
6. `gitmap-v26 exec -A web -- npm test` runs the command in the repo dir.
7. `gitmap-v26 cd -A infra` navigates to the repo directory.
8. `gitmap-v26 alias suggest` proposes aliases for unaliased repos.
9. `gitmap-v26 alias suggest --apply` auto-accepts all suggestions.
10. `gitmap-v26 scan` suggests aliases for newly discovered repos.
...
14. Alias names cannot shadow existing gitmap-v26 subcommands.
