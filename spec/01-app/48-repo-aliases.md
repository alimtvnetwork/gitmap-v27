...
repository. Once aliased, any `gitmap-v23` command can target the repo by
...
1. **Short names** — replace long slugs or paths with concise aliases
   (e.g., `api`, `web`, `infra`).
2. **Run from anywhere** — execute any gitmap-v23 command against a repo
...
```
gitmap-v23 alias set <alias> <slug-or-path>
gitmap-v23 alias remove <alias>
gitmap-v23 alias list
gitmap-v23 alias show <alias>
gitmap-v23 alias suggest [--apply]
```
...
Any gitmap-v23 command that operates on a repository accepts `-A <alias>`
...
gitmap-v23 exec -A web -- npm test

# Navigate to the aliased repo
gitmap-v23 cd -A infra

# Check status of an aliased repo
gitmap-v23 status -A api

# Watch an aliased repo
gitmap-v23 watch -A web
```
...
When `gitmap-v23 scan` or `gitmap-v23 rescan` discovers new repositories, the
system suggests aliases based on:

1. **Repo name** — last segment of the clone URL (e.g., `gitmap-v23`).
...
```bash
gitmap-v23 scan --apply-aliases    # Accept all suggestions automatically
gitmap-v23 alias suggest --apply   # Suggest for existing unaliased repos
```
...
```bash
gitmap-v23 completion --list-aliases
...
- Alias name conflicts with a gitmap-v23 subcommand → `alias cannot shadow command: <name>` (exit 1).

---

## Acceptance Criteria

1. `gitmap-v23 alias set api github/user/api-gateway` creates an alias.
2. `gitmap-v23 alias list` displays all aliases with their target slugs.
3. `gitmap-v23 alias show api` shows the linked repo details.
4. `gitmap-v23 alias remove api` deletes the alias.
5. `gitmap-v23 pull -A api` pulls the repo from any working directory.
6. `gitmap-v23 exec -A web -- npm test` runs the command in the repo dir.
7. `gitmap-v23 cd -A infra` navigates to the repo directory.
8. `gitmap-v23 alias suggest` proposes aliases for unaliased repos.
9. `gitmap-v23 alias suggest --apply` auto-accepts all suggestions.
10. `gitmap-v23 scan` suggests aliases for newly discovered repos.
...
14. Alias names cannot shadow existing gitmap-v23 subcommands.
