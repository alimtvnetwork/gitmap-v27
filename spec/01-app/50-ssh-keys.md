...
auto-managed in `~/.ssh/config`, and integrated with `gitmap-v26 clone` via
the `--ssh-key` flag.
...
1. **One-command key generation** — `gitmap-v26 ssh` creates an RSA-4096 key
...
3. **Quick access** — `gitmap-v26 ssh cat` prints the public key for copying.
...
6. **Clone integration** — `gitmap-v26 clone --ssh-key <name>` uses the
   named key's SSH config for the clone operation.

---

## Command: `gitmap-v26 ssh`
...
## Subcommand: `gitmap-v26 ssh` (Generate)
...
## Subcommand: `gitmap-v26 ssh list`
...
## Subcommand: `gitmap-v26 ssh delete`
...
## Subcommand: `gitmap-v26 ssh config`
...
When multiple keys exist, gitmap-v26 auto-manages a clearly marked block
in `~/.ssh/config`:

```
# --- gitmap-v26 managed (do not edit) ---
...
| `SSHKeyType`           | `"rsa"`                                      |
| `SSHKeyBits`           | `"4096"`                                     |
| `SSHConfigMarkerStart` | `"# --- gitmap-v26 managed (do not edit) ---"`   |
| `SSHConfigMarkerEnd`   | `"# --- end gitmap-v26 managed ---"`             |
...
- **See Also**: `gitmap-v26 clone`, `gitmap-v26 setup`
...
- After `gitmap-v26 ssh`: suggest subcommands (cat, list, delete, config).
- After `gitmap-v26 ssh cat --name`: suggest key names from DB via
  `gitmap-v26 completion --list-ssh-keys`.
- After `gitmap-v26 clone --ssh-key`: suggest key names.
...
1. `gitmap-v26 ssh` generates an RSA-4096 key with no passphrase and stores
   the public key in the database.
2. `gitmap-v26 ssh --name work --path ~/.ssh/id_rsa_work` creates a named
   key at a custom path.
3. `gitmap-v26 ssh cat` prints the default key's public key to stdout.
4. `gitmap-v26 ssh cat --name work` prints the named key's public key.
5. Running `gitmap-v26 ssh` when a key exists prompts for
   Regenerate/New path/Cancel (unless `--force`).
6. `gitmap-v26 ssh list` shows all stored keys with paths and fingerprints.
7. `gitmap-v26 ssh delete --name work` removes the DB record.
8. `gitmap-v26 ssh delete --name work --files` also removes key files.
9. `~/.ssh/config` is auto-updated with managed Host entries when
   keys are added or deleted.
10. `gitmap-v26 clone repos.json --ssh-key work` clones using the named
