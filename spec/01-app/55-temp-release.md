...
```
gitmap-v26 temp-release <count> <version-pattern> [-s <start>]
gitmap-v26 tr <count> <version-pattern> [-s <start>]
```
...
```bash
gitmap-v26 tr 10 v1.$$ -s 5 --dry-run
```
...
1. `gitmap-v26 tr 10 v1.$$ -s 5` creates 10 branches (`temp-release/v1.05`–`temp-release/v1.14`) from the last 10 commits, pushed to origin, without switching branches.
2. `gitmap-v26 tr 1 v1.$$` auto-detects the next sequence number from DB/remote and creates one branch.
3. `gitmap-v26 tr list` shows all temp-release branches with SHA, message, and date.
4. `gitmap-v26 tr list --json` outputs structured JSON.
5. `gitmap-v26 tr remove v1.05` prompts confirmation, then deletes local + remote branch.
6. `gitmap-v26 tr remove v1.05 to v1.10` prompts with full list, then deletes all in range.
7. `gitmap-v26 tr remove all` prompts with all branch names, then deletes everything.
8. `gitmap-v26 tr 10 v1.$$ --dry-run` shows preview without creating branches.
