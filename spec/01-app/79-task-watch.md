...
### `gitmap-v23 task` (alias: `tk`)

Manage and execute file-sync watch tasks.

```bash
gitmap-v23 task create <name> --src <path> --dest <path>
gitmap-v23 task list
gitmap-v23 task run <name>
gitmap-v23 task run <name> --interval 10
gitmap-v23 task delete <name>
gitmap-v23 task show <name>
```
...
```
gitmap-v23 task run frontend-sync — checking every 5s (Ctrl+C to stop)
...
```bash
# Create a task
gitmap-v23 task create ui-sync --src ./frontend/src --dest ./backend/static

# List all tasks
gitmap-v23 task list

# Run a task with default 5s interval
gitmap-v23 task run ui-sync

# Run with faster polling and verbose output
gitmap-v23 task run ui-sync --interval 2 --verbose

# Preview what would sync
gitmap-v23 task run ui-sync --dry-run

# Delete a task
gitmap-v23 task delete ui-sync
```
