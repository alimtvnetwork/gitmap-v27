...
The `gitmap-v26 install <tool>-settings` command family synchronises bundled
...
|--------------------------------|-------|------------------------------|--------------------------------------|
| `gitmap-v26 install npp`           | `in`  | `settings/01 - notepad++`    | Install Notepad++ **and** sync settings |
| `gitmap-v26 install npp-settings`  | `in`  | `settings/01 - notepad++`    | Sync Notepad++ settings only         |
| `gitmap-v26 install install-npp`   | `in`  | --                           | Install Notepad++ only (no settings) |
| `gitmap-v26 install vscode-settings` | `in` | `settings/02 - vscode`      | Sync VS Code settings and extensions |
| `gitmap-v26 install obs-settings`  | `in`  | `settings/03 - obs`          | Sync OBS Studio profiles and scenes  |
| `gitmap-v26 install wt-settings`   | `in`  | `settings/04 - windows-terminal` | Sync Windows Terminal settings.json |
...
After `gitmap-v26 install obs-settings`:
...
This ensures settings are found whether gitmap-v26 is run from the repo root,
...
| File                               | Purpose                                    |
|------------------------------------|--------------------------------------------|
| `gitmap-v26/cmd/installnpp.go`        | NPP settings sync + `resolveSettingsPath`  |
| `gitmap-v26/cmd/installnppextract.go` | NPP zip extraction logic                   |
| `gitmap-v26/cmd/installvscode.go`     | VS Code settings and extension sync        |
| `gitmap-v26/cmd/installobs.go`        | OBS zip extraction + scene/profile routing |
| `gitmap-v26/cmd/installwt.go`         | Windows Terminal settings sync             |
| `gitmap-v26/cmd/install.go`           | Command routing (`executeInstall`)         |
| `gitmap-v26/constants/constants_install.go` | Tool names and messages              |
...
- `gitmap-v26/helptext/install.md` -- CLI help text
