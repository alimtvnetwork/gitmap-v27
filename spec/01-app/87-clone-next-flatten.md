...
As of v2.75.0, `gitmap-v26 clone-next` (`cn`) **flattens by default** — it
...
The behavior also enables **version tracking** in the gitmap-v26 database:
...
```
gitmap-v26 cn <version-spec> [-f|--force] [--delete] [--keep] [--no-desktop] [--verbose]
```
...
| `-f` / `--force` | Same as default, but if cwd IS the target folder, gitmap-v26 chdirs to parent first, removes the cwd, and clones into the flattened name. **Never falls back to a versioned folder.** Aborts with a clear error if removal still fails. |
...
```
PS C:\repos\macro-ahk> gitmap-v26 cn v++ -f
...
`GITMAP_HANDOFF_FILE` (exported by the `gitmap-v26` shell wrapper function
...
Use `gitmap-v26 version-history` (`vh`) to see all recorded transitions:
