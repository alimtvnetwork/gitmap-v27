...
> Spec for the automatic binary sync step during `gitmap-v26 update`.

---

## Problem

On Windows, the compiled binary is deployed to a subdirectory (e.g., `E:\bin-run\gitmap-v26\gitmap.exe`), but the system PATH may resolve to a different copy at the parent level (e.g., `E:\bin-run\gitmap.exe`). After a successful build-and-deploy, the active PATH binary remains stale, producing:
...
1. Resolve the **active binary** via `Get-Command gitmap-v26` (PATH lookup).
...
4. Run `gitmap-v26 version` on both — if versions match, no sync is needed.
...
[HINT] Run 'gitmap-v26 doctor --fix-path' manually.
```
...
| All strategies fail              | `[WARN]` with hint to run `gitmap-v26 doctor --fix-path`         |
...
- [gitmap-v26/helptext/update.md](../../gitmap-v26/helptext/update.md) — User-facing update documentation
