...
`gitmap-v26 release-self` (alias `rs`, `rself`) provides explicit self-release capability.
It resolves the gitmap-v26 executable's own source repository and performs a full
release workflow from that directory, regardless of the user's current working
directory.

Additionally, `gitmap-v26 release` auto-detects when it is run outside a Git
...
### release-self (rs / rself)
    ...
    gitmap-v26 release-self [version] [flags]

Explicitly triggers a self-release. Accepts all flags supported by `release`.

### release (auto-fallback)

When `gitmap-v26 release` is invoked outside a Git repository, it automatically
...
`could not locate gitmap-v26 source repository`.
...
| Scenario | Behavior |
|----------|----------|
| Executable path unresolvable + no DB fallback | Exit 1: `could not locate gitmap-v26 source repository` |
...
1. `gitmap-v26 rs --bump patch` releases gitmap-v26 itself from any directory.
2. `gitmap-v26 release` outside a Git repo triggers self-release automatically.
3. `gitmap-v26 release` inside a Git repo behaves exactly as before.
