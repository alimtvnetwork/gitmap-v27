...
`gitmap-v23 release-self` (alias `rs`, `rself`) provides explicit self-release capability.
It resolves the gitmap-v23 executable's own source repository and performs a full
release workflow from that directory, regardless of the user's current working
directory.

Additionally, `gitmap-v23 release` auto-detects when it is run outside a Git
...
### release-self (rs / rself)
    ...
    gitmap-v23 release-self [version] [flags]

Explicitly triggers a self-release. Accepts all flags supported by `release`.

### release (auto-fallback)

When `gitmap-v23 release` is invoked outside a Git repository, it automatically
...
`could not locate gitmap-v23 source repository`.
...
| Scenario | Behavior |
|----------|----------|
| Executable path unresolvable + no DB fallback | Exit 1: `could not locate gitmap-v23 source repository` |
...
1. `gitmap-v23 rs --bump patch` releases gitmap-v23 itself from any directory.
2. `gitmap-v23 release` outside a Git repo triggers self-release automatically.
3. `gitmap-v23 release` inside a Git repo behaves exactly as before.
