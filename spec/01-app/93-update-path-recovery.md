...
The `update` command requires access to the gitmap-v23 source repository
...
7. On failure (path does not exist): clone the gitmap-v23 source repo
...
| Key                  | Value                              |
|----------------------|------------------------------------|
| `source_repo_path`   | Absolute path to the gitmap-v23 repo  |
...
1. `gitmap-v23 update --repo-path <valid-path>` saves the path to the
