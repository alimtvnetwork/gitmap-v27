...
The `update` command requires access to the gitmap-v26 source repository
...
7. On failure (path does not exist): clone the gitmap-v26 source repo
...
| Key                  | Value                              |
|----------------------|------------------------------------|
| `source_repo_path`   | Absolute path to the gitmap-v26 repo  |
...
1. `gitmap-v26 update --repo-path <valid-path>` saves the path to the
