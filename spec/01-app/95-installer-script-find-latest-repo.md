...
**Scope:** Generic. Applies to any project whose repo name ends in a `-v<N>` suffix (e.g. `gitmap-v26`, `myapp-v7`, `cli-tool-v12`).
...
This guarantees a stale install URL (e.g. someone bookmarked `gitmap-v26` years ago) still pulls the user onto the latest major repo line without manual intervention.
...
| **Installer path**| Path inside the repo to the installer (e.g. `gitmap-v26/scripts/install.ps1` or `install.sh`). MUST be identical across versions. |
...
| Input URL                                          | stem        | N  |
|----------------------------------------------------|-------------|----|
| `https://github.com/alimtvnetwork/gitmap-v26`       | `gitmap-v26`    | 3  |
...
| `gitmap-v26/scripts/install.ps1`    | ✅        | ✅         | `-NoDiscovery`, `-ProbeCeiling`  |
| `gitmap-v26/scripts/install.sh`     | ✅        | ✅         | `--no-discovery`, `--probe-ceiling` |
...
## 13. Concrete Example (gitmap-v26)
...
```
  [discovery] baseline: alimtvnetwork/gitmap-v26
  [discovery] probe ceiling: 30
  [discovery] HEAD https://github.com/alimtvnetwork/gitmap-v26 ... HIT
  [discovery] HEAD https://github.com/alimtvnetwork/gitmap-v26 ... MISS (fail-fast)
  [discovery] effective: alimtvnetwork/gitmap-v26
  [discovery] delegating to https://raw.githubusercontent.com/alimtvnetwork/gitmap-v26/main/install-quick.sh
```

The user transparently lands on `gitmap-v26` even though they invoked `gitmap-v26`.
