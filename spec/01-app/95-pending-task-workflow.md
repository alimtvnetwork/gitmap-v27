...
Every file-system operation in gitmap-v23 (delete, remove, scan, clone, pull,
...
automatically replayed via `gitmap-v23 do-pending`.
...
1. Re-execute `gitmap-v23 <CommandArgs>` as a subprocess.
...
5. `gitmap-v23 pending` lists all pending tasks with Id, type, path, reason.
6. `gitmap-v23 do-pending` retries all; `gitmap-v23 dp` is an alias.
7. `gitmap-v23 do-pending <id>` retries a single task.
