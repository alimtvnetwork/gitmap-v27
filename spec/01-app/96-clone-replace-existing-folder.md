...
**Status:** Draft
**Audience:** Any AI or human implementing the `gitmap-v23 clone <url>` flow (or any command that materialises a Git working copy into a target directory).
...
When `gitmap-v23 clone <url>` (or `clone-next`, or any flatten-style clone) targets a folder that **already exists** on disk, the current behaviour aborts with:
...
```
  ...
  [clone] swap complete; target now points at fresh clone (gitmap-v23)
...
| Target is the cwd of *this* gitmap-v23 process         | Same as user-shell cwd — strategy 2 handles it via empty-contents. |
...
`--replace` being default is the right call: every observed user invocation of `gitmap-v23 clone <url>` against an existing folder so far has meant "replace it".
...
This ensures `gitmap-v23 update-cleanup` can later sweep stale `*.gitmap-tmp-*` siblings.
...
- [ ] `gitmap-v23 clone <url>` against an existing folder no longer exits with "target folder already exists".
...
- [`gitmap-v23/cmd/clonenext.go`](../../gitmap-v23/cmd/clonenext.go) — Already implements a similar fallback for the flattened-folder lock case (v2.87.0+).
