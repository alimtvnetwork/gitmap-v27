# Move & Merge — `gitmap-v26 mv` / `merge-both` / `merge-left` / `merge-right`
...
```
gitmap-v26 mv          LEFT RIGHT     # move LEFT into RIGHT, delete LEFT
gitmap-v26 merge-both  LEFT RIGHT     # bidirectional fill: each side gains the other's missing files
gitmap-v26 merge-left  LEFT RIGHT     # take from RIGHT, put into LEFT
gitmap-v26 merge-right LEFT RIGHT     # take from LEFT,  put into RIGHT
```
...
```
gitmap-v26 mv https://github.com/owner/repo:develop ./local
```
...
1. **Folder does NOT exist:** clone the URL into that folder.
   Normal `gitmap-v26 clone <url>` semantics (see
...
5. If RIGHT originated from a URL: stage all changes, commit with
   `gitmap-v26 mv from <LEFT-display>`, and `git push`.
...
|---------|-------------------------|
| `mv` | `gitmap-v26 mv from <LEFT-display>` |
| `merge-both` | `gitmap-v26 merge-both with <other-display>` |
| `merge-left` | `gitmap-v26 merge-left from <RIGHT-display>` |
| `merge-right` | `gitmap-v26 merge-right from <LEFT-display>` |
...
```
gitmap-v26 mv          ./local https://github.com/owner/repo:release
gitmap-v26 merge-right ./local https://github.com/owner/repo:feature/x
```
...
gitmap-v26 mv ./gitmap-v26 https://github.com/alimtvnetwork/gitmap-v26

# move a remote repo's contents into a local folder
gitmap-v26 mv https://github.com/alimtvnetwork/gitmap-v26 ./another-folder

# move between two remote repos (clones both, copies, pushes RIGHT)
gitmap-v26 mv https://github.com/alimtvnetwork/gitmap-v26 \
         https://github.com/alimtvnetwork/gitmap-v26

# merge missing files only (identical or differing files prompt)
gitmap-v26 merge-both ./gitmap-v26 ./gitmap-v26

# merge with auto-accept: each side's source wins
gitmap-v26 merge-right ./gitmap-v26 https://github.com/alimtvnetwork/gitmap-v26 -y

# merge with explicit policy
gitmap-v26 merge-both ./gitmap-v26 https://github.com/alimtvnetwork/gitmap-v26 \
         -y --prefer-newer

# pin remote branch
gitmap-v26 merge-right ./local https://github.com/owner/repo:develop
...
```
  [mv] resolving LEFT  : ./gitmap-v26 (folder, exists)
  [mv] resolving RIGHT : https://github.com/alimtvnetwork/gitmap-v26
  [mv]   -> mapped to working folder: ./gitmap-v26
  [mv]   -> folder does not exist; cloning
  [mv]   -> clone OK (47 files, 1.2 MB)
  [mv] copying files LEFT -> RIGHT (excluding .git/) ...
  [mv]   copied 47 files
  [mv] deleting LEFT (./gitmap-v26) ...
  [mv]   deleted
  [mv] committing in RIGHT ...
  [mv]   commit 9a3c1e2 "gitmap-v26 mv from ./gitmap-v26"
  [mv] pushing RIGHT ...
  [mv]   push OK
  [mv] done
```
...
- [x] `gitmap-v26 mv folder-a folder-b` moves contents and deletes folder-a.
- [x] `gitmap-v26 mv folder url` clones url, copies files, commits + pushes.
- [x] `gitmap-v26 mv url folder` clones url, copies files into folder, deletes the cloned working folder.
- [x] `gitmap-v26 mv url-a url-b` clones both, copies, pushes RIGHT, deletes LEFT clone.
...
> **Implementation:** v2.98.0 — `gitmap-v26/movemerge/` package, `cmd/move.go`, `cmd/merge.go`, `cmd/movemergeflags.go`, `cmd/dispatchmovemerge.go` wired in `cmd/root.go`. Helptext lives in `gitmap-v26/helptext/{mv,merge-both,merge-left,merge-right}.md` (added in v2.96.0).
