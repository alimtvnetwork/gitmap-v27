...
lock-step with the gitmap-v26 database so every scanned repo or path the user
explicitly registers via `gitmap-v26 code` shows up immediately in the VS Code
...
{
  "name": "gitmap-v26",
  "rootPath": "d:\\wp-work\\riseup-asia\\gitmap-v26",
  "paths": [],
  "tags": [],
  "enabled": true,
  "profile": ""
...
| Field      | Type      | gitmap-v26 behavior                                                  |
|------------|-----------|------------------------------------------------------------------|
| `name`     | string    | DB alias. On first insert: folder basename. Updated by `gitmap-v26 as`. |
...
- API: `gitmap-v26 code <alias> <root> [extra...]` (variadic, additive)
  and `gitmap-v26 code paths add|rm|list <alias> [path]` (explicit).
...
- `gitmap-v26 as <newalias>` only rewrites `name`. Multi-root paths, tags,
...
- Opt-out per scan: `gitmap-v26 scan --no-auto-tags`.
...
If the root directory does not exist, gitmap-v26 reports a clear error
...
If the file does not exist, gitmap-v26 creates it with `[]`. If the parent
directory does not exist, gitmap-v26 returns an error rather than creating
...
### 5.1 `gitmap-v26 scan` — auto-sync (default ON)

After the existing scan + DB upsert phase, gitmap-v26 reads every
`VSCodeProject` row and reconciles `projects.json`:

- New `rootPath` → append entry with gitmap-v26 defaults.
...
### 5.2 `gitmap-v26 code [alias] [path]` — register + open

| Invocation                  | Behavior                                                                 |
|-----------------------------|--------------------------------------------------------------------------|
| `gitmap-v26 code`               | Use git repo root (if inside one) else CWD; alias = folder basename.     |
| `gitmap-v26 code myalias`       | Same path resolution; alias overridden to `myalias`.                     |
| `gitmap-v26 code myalias /path` | Use `/path` (any path, no git requirement); alias = `myalias`.           |
...
### 5.3 `gitmap-v26 as <newalias>` — alias rename mirror

Existing `gitmap-v26 as` flow gains a post-hook: after the DB rename succeeds,
it calls the same projects.json sync helper so the matching `rootPath` row
gets its `name` updated. No new flag.

## 6. Database

Extends the unified gitmap-v26 SQLite DB at the binary path
...
All new strings land in `gitmap-v26/constants/constants_vscode.go`
...
| `code` CLI missing in PATH (gitmap-v26 code)   | actionable install hint above                                            |
...
3. `gitmap-v26 code` inside a git repo → repo root added, alias = basename, VS Code opens.
4. `gitmap-v26 code myalias` → alias overridden to `myalias`, VS Code opens.
5. `gitmap-v26 code myalias D:\anywhere` → non-git path added and opened.
6. `gitmap-v26 as newalias` mirrors `name` change to `projects.json`.
...
### `gitmap-v26 code [alias] [path]`

```
user → gitmap-v26 code [alias] [path]
        │
        ▼
...
- `gitmap-v26/constants/constants_vscode.go` (existing executable discovery constants)
