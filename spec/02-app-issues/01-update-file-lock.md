...
`gitmap-v26 update` fails with "file is being used by another process" because the active `gitmap.exe` process holds file handles when the deployment step attempts to overwrite it via `Copy-Item`.
...
1. **Copy-and-handoff** (`gitmap-v26/cmd/update.go`):
...
4. **Auto-cleanup** (generated PowerShell script):
   - After successful update, runs `gitmap-v26 update-cleanup`
