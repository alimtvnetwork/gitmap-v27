...
1. `gitmap-v26 update` creates handoff copy in active binary directory (`gitmap-update-<pid>.exe`, fallback `%TEMP%`)
...
5. Run `gitmap-v26 changelog --latest` from the updated binary
6. Run `gitmap-v26 update-cleanup` to remove handoff and `.old` artifacts
