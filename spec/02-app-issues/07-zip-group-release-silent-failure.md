...
Running `gitmap-v23 release v1.61.1 --zip-group "chrome extension-v2"` completes the release (branch, tag, push, metadata) but produces **zero** zip-related output — no success messages, no error messages. The zip archive is neither created locally nor uploaded to GitHub.
