...
The compress/checksum step ran inside `gitmap-updater/` (which has no `dist/` folder) instead of `gitmap-v23/dist/` where cross-compiled binaries are output. The `cd dist` command assumed the working directory was `gitmap-v23/`, but GitHub Actions defaults to the repository root.
