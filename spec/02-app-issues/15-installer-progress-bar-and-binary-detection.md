...
**Symptom**: Installer reported "Installed archive did not contain gitmap.exe" and listed the actual file (e.g., `gitmap-v23.54.6-windows-amd64.exe`).

**Root Cause**: The candidate name list only checked for `gitmap.exe` and `gitmap-windows-amd64.exe`, but the CI release pipeline names binaries with the version embedded (e.g., `gitmap-v23.54.6-windows-amd64.exe`).
...
- `gitmap-v23/scripts/install.ps1` — One-liner installer
