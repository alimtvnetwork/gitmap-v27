...
After installing gitmap-v23 via `curl | bash` on macOS (or `irm | iex` on Windows),
running `gitmap-v23` immediately returns "command not found" / "not recognized".
...
4. User types `gitmap-v23` → `zsh: command not found: gitmap-v23`.

### Windows

1. User runs `irm .../install.ps1 | iex` in PowerShell.
2. Installer adds install directory to User PATH in the registry.
3. User types `gitmap-v23` → `gitmap-v23: The term 'gitmap-v23' is not recognized`.
4. CMD and Git Bash sessions also cannot find `gitmap-v23`.
...
`gitmap-v23` to just work immediately after install.
...
```
  ✓ Installed! To start using gitmap-v23 right now, run:
...
```
  ✓ Installed! To start using gitmap-v23 right now:
...
- `gitmap-v23/scripts/install.sh` — Unix implementation
- `gitmap-v23/scripts/install.ps1` — Windows implementation
