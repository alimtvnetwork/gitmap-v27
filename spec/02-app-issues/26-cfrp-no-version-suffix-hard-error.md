...
> flag in `gitmap-v26/cmd/clonefixrepo.go`.

## Problem

Running `gitmap-v26 clone-fix-repo-pub <url>` (alias `cfrp`) on a URL whose
...
1. **User confusion.** `cfrp` was conceived for the versioned-repo
   workflow (`gitmap-v26`, `coding-guidelines-v23`). Running it on a
...
| Scenario | Behavior |
|---|---|
| URL ends in `-v<N>` (e.g. `gitmap-v26`) | Run the full pipeline. |
...
✗ cfrp: <url> is not a versioned repository (no -v<N> suffix)
  cfrp expects a sibling-versioned repo (gitmap-v26, coding-guidelines-v23, ...).
  To proceed anyway (skip the rewrite, still flip visibility public):
      gitmap-v26 cfrp <url> --require-version=false
```
