...
## Issue 1: `gitmap-v23 hd` Fails — docs-site Not Included in Release

### Symptom

```
PS D:\scripts-fixture> gitmap-v23 hd
...
## Issue 2: Directory Casing — `GitMap` vs `gitmap-v23`

### Symptom

The error path shows `D:\dev\GitMap\docs-site` — PascalCase `GitMap` directory.
`constants_doctor.go` correctly uses `GitMapSubdir = "gitmap-v23"` (lowercase), but
...
Ensure all references use lowercase `gitmap-v23` consistently. The `resolveBinaryDir()`
