...
**Affects:** `gitmap-v26 help-dashboard` (`gitmap-v26 hd`) on every install path
...
1. Local source build → `run.ps1` deploy to `$env:DeployPath\gitmap-v26\`
2. Local source build → `run.sh` deploy to `$DEPLOY_TARGET/gitmap-v26/`
3. Remote release install → `install.ps1` / `install.sh` to `$LOCALAPPDATA\gitmap-v26` / `~/.local/bin`

---

## Root Cause

`gitmap-v26 help-dashboard` resolves the docs folder relative to the binary directory
...
- [ ] `run.ps1` deploy logs `Copied docs-site/dist to gitmap-v26 app directory`
- [ ] `<deploy>\gitmap-v26\docs-site\dist\index.html` exists
- [ ] `gitmap-v26 hd` opens `http://localhost:5173` with the static dist served
- [ ] `install.ps1` (one-liner) logs `Installed docs-site to <installDir>\docs-site`
- [ ] `install.sh` mirrors the same on Linux/macOS
- [ ] Releases without `docs-site.zip` print `skipping (gitmap-v26 hd may not work)`
...
- `gitmap-v26/scripts/install.ps1` — `Install-DocsSite`, called in `Main`
- `gitmap-v26/scripts/install.sh` — `install_docs_site`, called in `main`
- `gitmap-v26/cmd/helpdashboard.go` — auto-extract fallback (unchanged)
- `gitmap-v26/release/workflowdocs.go` — release-side bundling (unchanged)
