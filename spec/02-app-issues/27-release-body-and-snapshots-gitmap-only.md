# `gitmap-v26 release` must not leak gitmap-specific content into other repos' releases

## Symptom

Running `gitmap-v26 release` inside a non-gitmap repository (e.g. `img-pdf-v2`,
...
1. **Attached `release-version-vX.Y.Z.ps1` / `release-version-vX.Y.Z.sh`
   ...
   `REPO="alimtvnetwork/gitmap-v26"` and `BINARY_NAME="gitmap-v26"`.
   A user who copy-pasted that one-liner would download and install the
   gitmap-v26 binary into the wrong repo's release flow.

2. **Dumped gitmap-v26's `CHANGELOG.md` notes into the release body** because
...
- `gitmap-v26/release/workflowfinalize.go::pushAndFinalize` —
...
- `gitmap-v26/release/workflowgithub.go::uploadToGitHub` — the release body
...
For the canonical gitmap-v26 source repos (`gitmap-v26`, `gitmap-v26`, …):
...
- `gitmap-v26/release/releaseinstallhint.go::ShouldPrintInstallHint` — the
  single source of truth for "is this the gitmap-v26 source repo?".
