...
> - `gitmap-v23/scripts/install.sh`, `install-quick.sh`
...
OK To start using gitmap-v23 right now, run:
...
Installed to:
gitmap-v23 quick installer
---------------------
Choose install folder. Press Enter to accept the default.
Default: /Users/ab_mahin/.local/bin
Install path: /Users/ab_mahin/.local/bin/gitmap-cli/gitmap-v23   ← prompt fires AGAIN
App folder on PATH: gitmap-v23 quick installer                   ← garbage label
---------------------
Choose install folder. Press Enter to accept the default.
Default: /Users/ab_mahin/.local/bin
Install path: /Users/ab_mahin/.local/bin/gitmap-cli           ← prompt fires a THIRD time

Done! Run 'gitmap-v23 --help' to get started.

PS /Users/ab_mahin/Downloads/core-v8> gitmap-v23 r v1.5.6
gitmap-v23: The term 'gitmap-v23' is not recognized as a name of a cmdlet, function, ...
PS /Users/ab_mahin/Downloads/core-v8> gitmap-v23
gitmap-v23: The term 'gitmap-v23' is not recognized as a name of a cmdlet, function, ...
PS /Users/ab_mahin/Downloads/core-v8> gitmap-v23 --help
gitmap-v23: The term 'gitmap-v23' is not recognized as a name of a cmdlet, function, ...
```
...
1. **Wrong-shell PATH wiring** — install.sh wrote PATH to `.zshrc` /
   ...
   `gitmap-v23` is not on PATH in the active session OR in any future pwsh
   session.
...
`gitmap-v23/scripts/install.sh::add_to_path` detects the active shell with:
...
`gitmap-v23/constants/constants_pathsnippet.go::PathSnippetPwshFmt`), but
...
The "garbage label" `App folder on PATH: gitmap-v23 quick installer` is
...
3. Extend `add_to_path` with a pwsh arm that calls
   ...
   - Renders the snippet via `gitmap-v23 setup print-path-snippet
     --shell pwsh --dir <dir> --manager installer`.
   - Uses the same `# gitmap-v23 shell wrapper v2 ...` marker block, so
...
PS> . $PROFILE
PS> gitmap-v23 --version
# expect: prints v3.43.1
```

Automated regression (CI, future work):

- New test in `gitmap-v23/scripts/install_test.sh` (does not exist yet)
