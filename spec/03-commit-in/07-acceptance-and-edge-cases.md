...
1. `gitmap-v23 commit-in <s> a b c`, `gitmap-v23 commit-in <s> a,b,c`,
   `gitmap-v23 commit-in <s> "a, b, c"`, and
   `gitmap-v23 commit-in <s> "a"  "b"   "c"` produce identical
...
4. `all` from source basename `gitmap-v23` expands to `gitmap-v23, gitmap-v23,
   gitmap-v23, …, gitmap-v23` (ascending; plain `gitmap-v23` first as `v0`).
...
19. Every enum in §02–§06 lives in its own Go file under
    `gitmap-v23/cmd/commitin/<enumname>.go`, used by both production code
...
`gitmap-v23/cmd/commitin/integration_test.go` that build a synthetic input
