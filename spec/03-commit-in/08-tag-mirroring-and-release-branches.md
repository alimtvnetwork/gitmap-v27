...
branches are interchangeable with `gitmap release-branch` and friends.
...
This is exactly the regex used by `gitmap release` for release-branch
...
- **`<200` lines per file**: the new package
  `gitmap/cmd/commitin/replay/tags.go` holds the orchestration only;
  git plumbing lives in `gitmap/gitutil/tags.go` (annotated-tag
  read/write helpers).
