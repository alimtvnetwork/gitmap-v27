...
branches are interchangeable with `gitmap-v23 release-branch` and friends.
...
This is exactly the regex used by `gitmap-v23 release` for release-branch
...
- **`<200` lines per file**: the new package
  `gitmap-v23/cmd/commitin/replay/tags.go` holds the orchestration only;
  git plumbing lives in `gitmap-v23/gitutil/tags.go` (annotated-tag
  read/write helpers).
