...
gitmap-v26 binary location, and integrates Linux/macOS shell-based update
...
```
  ...
  Binary:    /home/alim/.local/bin/gitmap-v26
...
The `gitmap-v26 update` command's `executeUpdate` function only supported
...
1. Resolves the active binary's install directory using `resolveInstalledDir()`:
   - First tries `exec.LookPath("gitmap-v26")` to find the PATH binary.
...
3. `run.sh` already handles:
   ...
   - **PATH binary sync** (lines 601-618): if the active `which gitmap-v26`
...
|----------|--------|-------------|
| 1 | `exec.LookPath("gitmap-v26")` | Finds the binary on PATH |
...
| File | Change |
|------|--------|
| `gitmap-v26/cmd/installeddir.go` | New: `runInstalledDir()` command |
| `gitmap-v26/cmd/updatescript.go` | Added `executeUpdateUnix()`, `resolveInstalledDir()` |
| `gitmap-v26/cmd/rootutility.go` | Added `installed-dir` / `id` dispatch |
| `gitmap-v26/constants/constants_cli.go` | Added `CmdInstalledDir`, `CmdInstalledDirAlias`, `HelpInstalledDir` |
| `gitmap-v26/constants/constants_update.go` | Added `ErrUpdateNoRunSH`, `MsgUpdateInstallDir` |
| `gitmap-v26/helptext/installed-dir.md` | New: command documentation |

---

## 5. Acceptance Criteria

1. `gitmap-v26 installed-dir` prints the binary path and directory.
2. `gitmap-v26 id` produces the same output.
3. `gitmap-v26 update` on Linux/macOS runs `run.sh --update` from the source repo.
4. After update, `gitmap-v26 version` reflects the latest version.
