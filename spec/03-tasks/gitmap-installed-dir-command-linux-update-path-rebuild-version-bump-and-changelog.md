...
gitmap-v23 binary location, and integrates Linux/macOS shell-based update
...
```
  ...
  Binary:    /home/alim/.local/bin/gitmap-v23
...
The `gitmap-v23 update` command's `executeUpdate` function only supported
...
1. Resolves the active binary's install directory using `resolveInstalledDir()`:
   - First tries `exec.LookPath("gitmap-v23")` to find the PATH binary.
...
3. `run.sh` already handles:
   ...
   - **PATH binary sync** (lines 601-618): if the active `which gitmap-v23`
...
|----------|--------|-------------|
| 1 | `exec.LookPath("gitmap-v23")` | Finds the binary on PATH |
...
| File | Change |
|------|--------|
| `gitmap-v23/cmd/installeddir.go` | New: `runInstalledDir()` command |
| `gitmap-v23/cmd/updatescript.go` | Added `executeUpdateUnix()`, `resolveInstalledDir()` |
| `gitmap-v23/cmd/rootutility.go` | Added `installed-dir` / `id` dispatch |
| `gitmap-v23/constants/constants_cli.go` | Added `CmdInstalledDir`, `CmdInstalledDirAlias`, `HelpInstalledDir` |
| `gitmap-v23/constants/constants_update.go` | Added `ErrUpdateNoRunSH`, `MsgUpdateInstallDir` |
| `gitmap-v23/helptext/installed-dir.md` | New: command documentation |

---

## 5. Acceptance Criteria

1. `gitmap-v23 installed-dir` prints the binary path and directory.
2. `gitmap-v23 id` produces the same output.
3. `gitmap-v23 update` on Linux/macOS runs `run.sh --update` from the source repo.
4. After update, `gitmap-v23 version` reflects the latest version.
