...
> - App reference: [spec/03-general/02d-deploy-patterns.md](../03-general/02d-deploy-patterns.md) — gitmap-v26 nested deploy pattern that motivates this generic spec
...
(e.g. `E:\gitmap.exe` -> `E:\bin-run\gitmap-v26\gitmap.exe`). One install,
...
| Capability | `run.ps1` (Windows dev) | `run.sh` (Unix dev) | `gitmap-v26/scripts/install.sh` (end-user) |
|------------|-------------------------|---------------------|----------------------------------------|
| **DFD-1** Wrapped layout `<root>/gitmap-cli/gitmap-v26(.exe)` | `Deploy-Binary` → `$appDir = Join-Path $target "gitmap-cli"` | `deploy_binary()` → `app_dir="$target/gitmap-cli"` (v3.13.11+) | `install_binary()` → `INSTALL_DIR/gitmap-cli/` (v3.13.11+) |
| **DFD-2** Resolve target from PATH first | `Resolve-DeployTarget` walks `Get-Command gitmap-v26`; accepts `gitmap-cli` or legacy `gitmap-v26` parent | `resolve_deploy_target()` walks `command -v gitmap-v26`; accepts `gitmap-cli` or legacy `gitmap-v26` parent | Honors `--prefix` then `$HOME/.local` default |
| **DFD-3** Migrate legacy unwrapped install + legacy `gitmap-v26/` folder | `Repair-DeployLayout` (handles unwrapped + `gitmap-v26` → `gitmap-cli`) | `repair_deploy_layout()` (handles unwrapped + `gitmap-v26` → `gitmap-cli`, v3.13.11+) | `repair_layout()` (handles unwrapped + `gitmap-v26` → `gitmap-cli`, v3.13.11+) |
...
| **DFD-9** Persist resolved target back to config | `Sync-ConfigDeployPath` rewrites `powershell.json` | `sync_config_deploy_path()` rewrites `gitmap-v26/shell.json` | n/a (end-user installer is config-less) |
...
| macOS (zsh) | `gitmap-v26/scripts/install.sh` | ✅ v2.94.0 | `~/.local/bin/gitmap-v26/` layout, `~/.zshrc` snippet. |
...
- The job kept regressing on legitimate cross-platform divergences
  (e.g. `gitmap-cli/` vs legacy `gitmap-v26/`) and produced more red builds
...
1. **`gitmap-v26 doctor`** — every user runs this on first launch and after
   ...
   (`gitmap-v26/constants/deploy-manifest.json`), and reports actionable
   fixes via `gitmap-v26 doctor --fix-path`.
...
and add a **targeted** unit test to `gitmap-v26/cmd/*_test.go` for the
