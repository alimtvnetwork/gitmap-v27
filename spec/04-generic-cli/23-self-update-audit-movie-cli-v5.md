...
**Reference**: gitmap-v23 repo (this codebase), `gitmap-v23/cmd/update.go`,
`gitmap-v23/cmd/updatescript.go`, `run.ps1` (`-Update` branch).
...
language) that mirror `gitmap-v23/cmd/update.go` lines 18–157. Do **not** keep
...
check it first. See `gitmap-v23/cmd/update.go` `resolveRepoPath()`.
...
See `gitmap-v23/cmd/updatescript.go::writeScriptToTemp` (BOM = `0xEF 0xBB 0xBF`).
...
4. **Implement Go `movie update` + hidden `movie update-runner`** mirroring
   `gitmap-v23/cmd/update.go`. Use foreground `cmd.Run()` (D1, D9).
...
- Read `gitmap-v23/cmd/update.go`, `gitmap-v23/cmd/updatescript.go`, and the
