...
**Reference**: gitmap-v26 repo (this codebase), `gitmap-v26/cmd/update.go`,
`gitmap-v26/cmd/updatescript.go`, `run.ps1` (`-Update` branch).
...
language) that mirror `gitmap-v26/cmd/update.go` lines 18–157. Do **not** keep
...
check it first. See `gitmap-v26/cmd/update.go` `resolveRepoPath()`.
...
See `gitmap-v26/cmd/updatescript.go::writeScriptToTemp` (BOM = `0xEF 0xBB 0xBF`).
...
4. **Implement Go `movie update` + hidden `movie update-runner`** mirroring
   `gitmap-v26/cmd/update.go`. Use foreground `cmd.Run()` (D1, D9).
...
- Read `gitmap-v26/cmd/update.go`, `gitmap-v26/cmd/updatescript.go`, and the
