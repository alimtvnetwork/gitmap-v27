...
release row to the repository it belonged to. This worked while gitmap-v23 held
...
4. If no row matches (the repo has never been scanned), the release write
   **fails loudly** with a clear message instructing the user to run
   `gitmap-v23 scan` first. We do NOT auto-create a stub `Repo` row — release
...
2. **Warn**: print to `os.Stderr`:
   ...
   → Existing Release rows will be wiped and re-imported on next
     `gitmap-v23 list-releases` or `gitmap-v23 scan-import`.
   ```
...
5. Next `gitmap-v23 list-releases` invocation calls `loadReleasesFromRepo()`
...
Each gitmap-v23 database still tracks **one repo's releases at a time** — the
...
**However**, the schema now permits a future `gitmap-v23 releases --all-repos`
...
| Risk | Mitigation |
|------|------------|
| User has `Release` rows but no `.gitmap/release/` files | Lost. Documented in CHANGELOG breaking-change note. Recovery: `gitmap-v23 release-import --from-github` (existing command). |
| Concurrent `gitmap-v23` process holds DB lock during migration | Existing `gitmap.lock` advisory lock blocks concurrent writers. |
| Repo not yet scanned when first release fires | UpsertRelease returns a clear error: "run `gitmap-v23 scan` first". Aborts cleanly. |
