...
`gitmap-v23 fix-repo` (alias `gitmap-v23 fr`) rewrites prior versioned-repo-name
...
point and is shipped inside the `gitmap-v23` binary.

## Synopsis

```
gitmap-v23 fix-repo [-2 | -3 | -5 | --all] [--dry-run] [--verbose] [--config <path>]
...
- Short alias: `fr`.
- Constants live in `gitmap-v23/constants/constants_fixrepo.go` (package-
  domain ownership, per the constants-ownership rule).
- Implementation is split across `gitmap-v23/cmd/fixrepo*.go` files,
...
### Before / after table — `gitmap-v23 fix-repo` semantics by current version

| Working repo | Target span | Token in source       | Rewritten to        | Why |
|--------------|-------------|-----------------------|---------------------|-----|
| `gitmap-v23`  | v1          | `gitmap-v23`              | `gitmap-v23`         | Bare sweep ACTIVE (n=1, current=2) |
| `gitmap-v23`  | v1          | `gitmap-v23`           | `gitmap-v23`         | Versioned token rewrite |
| `gitmap-v23`  | v1          | `gitmap.js`           | `gitmap.js`         | Word-boundary guard |
| `gitmap-v23`  | v1 (`-r nv`)| `gitmap-v23`              | `gitmap-v23`            | Bare sweep suppressed by `--restrict no-version` |
| `gitmap-v23`  | v1, v2      | `gitmap-v23`              | `gitmap-v23`            | Bare sweep SKIPPED (current ≥ 3) |
| `gitmap-v23`  | v1, v2      | `gitmap-v23`/`-v2`     | `gitmap-v23`         | Versioned token rewrite |
| `gitmap-v23`  | v1..v3      | `https://…/gitmap-v23`    | `https://…/gitmap-v23`  | Bare upstream URL preserved |
| `gitmap-v23`  | v1..v3      | `gitmap-v23`          | `gitmap-v23`        | Negative-lookahead vs `-v1` |
| `gitmap-v23`  | v1..v3      | `gitmap-v23..v3`       | `gitmap-v23`         | Versioned token rewrite |

Pre-v5.38.0 regression: running inside `gitmap-v23` rewrote every bare
`gitmap-v23` mention (binary name, upstream URL, `gitmap-cli` package
descriptor, brand strings) to `gitmap-v23`. The v5.38.0 gate restored
correctness; the table above is the contract going forward.

Implementation: `applyAllTargets` in `gitmap-v23/cmd/fixrepo_rewrite.go`,
...
```
gitmap-v23 fix-repo -2 --restrict no-version
gitmap-v23 fr -2 -r nv
```

Implementation: `applyAllTargetsR` in
`gitmap-v23/cmd/fixrepo_rewrite.go` gates the bare-base call on
...
Every non-dry-run `gitmap-v23 fix-repo` invocation that modifies ≥ 1 file
...
2. **Scoped to `<repo>/v<current>`.** An `undo` inside `gitmap-v23`
   never sees a `gitmap-v23` snapshot. This prevents cross-version
...
3. **Lazy + idempotent.** The snapshot directory is created on the
   ...
   original backup).
4. **Manifest-driven restore.** `gitmap-v23 undo` reads `manifest.json`
   and copies each listed `files/<rel>` back to `<repoRoot>/<rel>`.

### `gitmap-v23 undo` (alias `ud`)

```
gitmap-v23 undo                       # restore latest snapshot
gitmap-v23 undo --list                # list snapshots, newest first (* = latest)
gitmap-v23 undo --snapshot <UTC-ts>   # restore a specific stamp
gitmap-v23 undo --dry-run             # preview without writing
```

Exit codes: `0` ok / `6` bad-flag / `7` write-failed / `8` bad-config
(manifest missing or malformed).

Implementation: `gitmap-v23/cmd/fixrepo_backup.go` (session +
`BackupFile` / `Finalize`), `gitmap-v23/cmd/undo.go` (list / pick /
