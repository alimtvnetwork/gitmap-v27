...
Every gitmap-v23 operation that **mutates the on-disk filesystem** is recorded
...
gitmap-v23 state, so it is automatically anchored to the binary path
...
## 5. CLI surface — extend `gitmap-v23 revert`

The legacy `gitmap-v23 revert <version>` flow (rolls back the gitmap-v23
...
Help text lives at `gitmap-v23/helptext/revert.md` (extended, not replaced).
...
- `gitmap-v23 revert --prune-txn` lets users force a prune cycle.

## 8. Concurrency

- The existing `gitmap.lock` advisory file lock guards every Begin
  / Commit / Revert call so two gitmap-v23 processes never interleave
  transaction rows.
...
- A full UI under `gitmap-v23 txn ...` — left as a follow-up; the `revert`
  flag surface is sufficient for v1.

## 10. Implementation references

- Schema: `gitmap-v23/constants/constants_transaction.go`
- Tables: `gitmap-v23/store/transaction.go`, `gitmap-v23/store/transaction_file.go`
- Engine: `gitmap-v23/txn/begin.go`, `gitmap-v23/txn/snapshot.go`, `gitmap-v23/txn/revert.go`, `gitmap-v23/txn/prune.go`
- CLI: `gitmap-v23/cmd/revert.go` (extended), `gitmap-v23/cmd/revert_txn.go` (new)
- Help: `gitmap-v23/helptext/revert.md` (extended)
