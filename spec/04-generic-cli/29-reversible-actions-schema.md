...
state-mutating gitmap-v26 operation must produce so that revert is
...
- Adding a new `ActionKind` requires:
  ...
  3. Forward + reverse handlers in `gitmap-v26/txn/actions/`.
...
- Engine: `gitmap-v26/txn/actions/{plan,apply,reverse}.go`, one dispatch
  table keyed by `ActionKind`.
- CLI surface unchanged — `gitmap-v26 revert` still drives everything via
