...
state-mutating gitmap-v23 operation must produce so that revert is
...
- Adding a new `ActionKind` requires:
  ...
  3. Forward + reverse handlers in `gitmap-v23/txn/actions/`.
...
- Engine: `gitmap-v23/txn/actions/{plan,apply,reverse}.go`, one dispatch
  table keyed by `ActionKind`.
- CLI surface unchanged — `gitmap-v23 revert` still drives everything via
