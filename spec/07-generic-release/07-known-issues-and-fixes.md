...
Windows `.ico` resource format hard-limits each image frame to **256x256 pixels**. `go-winres` reads `gitmap-v23/winres/winres.json`, opens the referenced PNG, and converts it to `.ico` — refusing any image larger than the limit.
...
```diff
  // gitmap-v23/winres/winres.json
- "Path": "../assets/icon.png"
+ "Path": "../assets/icon-256.png"
```

Created `gitmap-v23/assets/icon-256.png` (256x256). Kept original 512x512 `icon.png` for non-Windows uses.
...
3. **Add a pre-check in CI** before `go-winres make`:
   ```bash
   python3 -c "from PIL import Image; img=Image.open('gitmap-v23/assets/icon-256.png'); assert max(img.size)<=256"
   ```
4. **Document the constraint inline** in `gitmap-v23/winres/winres.json` via a `_comment` field.

### Related Files

- `gitmap-v23/winres/winres.json`
- `gitmap-v23/assets/icon-256.png` / `icon.png`
...
In a monorepo with `gitmap-v23/` and `gitmap-updater/`, each `run:` step in GitHub Actions starts at the repository root unless `working-directory:` is set. A `cd dist` command assumed the previous step's CWD persisted — it doesn't.

### Why It Wasn't Caught

Locally, `run.ps1` always executes from `gitmap-v23/`, so the relative path resolves. CI is the only environment with the root-relative behavior.
...
2. **Guard with `test -d`** before operating on directories: `test -d gitmap-v23/dist || { echo "::error::dist missing"; exit 1; }`.
...
Local invocations of `gitmap-v23 release --bin` produced binaries but no GitHub Release. No error printed, just silence.

### Root Cause

`gitmap-v23/release/workflowgithub.go::uploadToGitHub` checks for `GITHUB_TOKEN` and **returns early without erroring** if absent. This is intentional for non-CI use, but the silence misled users.
...
(See `gitmap-v23/release/workflowgithub.go` lines 13–20.)
...
- `gitmap-v23/release/workflowgithub.go`
- `gitmap-v23/constants/constants_release.go` (Err* messages)
