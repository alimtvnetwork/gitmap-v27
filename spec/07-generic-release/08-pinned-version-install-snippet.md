...
> **Status:** Active since gitmap-v26 **v3.12.0** (2026-04-20).
...
`gitmap-v26/release/installsnippet.go::AppendPinnedInstallSnippet` runs inside
...
```powershell
$ver = 'v3.11.1'
$installer = irm https://raw.githubusercontent.com/alimtvnetwork/gitmap-v26/main/gitmap-v26/scripts/install.ps1
...
```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v26/main/gitmap-v26/scripts/install.sh \
...
* `gitmap-v26/scripts/install.ps1` lines around the `INSTALLER_DELEGATED`
  branch — new `elseif (-not [string]::IsNullOrWhiteSpace($Version))`
  arm prints `[discovery] -Version <tag> pinned; skipping repo probe`.
* `gitmap-v26/scripts/install.sh` lines around the same branch — new
...
1. **Bump version** in `gitmap-v26/constants/constants.go` and
...
4. **Run `gitmap-v26 release`**. The publisher auto-appends the pinned
   snippet — do not paste it manually.
5. **Verify on the release page** that the snippet renders the correct
   tag (e.g. `v3.12.0`) and points at `gitmap-v26` (not `gitmap-v26`).
...
| Version | Change |
|---|---|
| v3.12.0 | Initial spec + implementation. Snippet auto-appended; both installers honor `--version` / `-Version` to skip discovery + latest lookup. Repo renamed `gitmap-v26` → `gitmap-v26` everywhere. |
