# JSON Schema docs for `gitmap-v23` CLI outputs
...
1. **Validate** that a `gitmap-v23 <cmd> --json` output conforms to the documented
...
unordered per RFC 8259. However, `gitmap-v23`'s stablejson outputs DO emit keys in
a contractually fixed order (see `gitmap-v23/stablejson/stablejson.go`), and some
...
|---|---|---|---|
| `gitmap-v23 startup-list --json` | [`startup-list.schema.json`](startup-list.schema.json) | **strict** | Backed by `gitmap-v23/stablejson`; key order is contractual |
| `gitmap-v23 list-releases --json` | [`list-releases.schema.json`](list-releases.schema.json) | **strict** | Per-repo view; lowerCamel keys mirror `model.ReleaseRecord` `json:` tags; pinned by `gitmap-v23/cmd/listreleases_jsonschema_contract_test.go` |
| `gitmap-v23 list-releases --all-repos --json` | [`list-releases-all-repos.schema.json`](list-releases-all-repos.schema.json) | **strict** | Joined Release+Repo view; PascalCase keys preserve the legacy `json.MarshalIndent` surface (`store.ReleaseAcrossRepos` has no `json:` tags) |
| `gitmap-v23 scan` (JSON + CSV)   | [`scan-output.schema.json`](scan-output.schema.json) + [`scan-output.README.md`](scan-output.README.md) | **field-name stable** | Field names/keys are permanent; new fields are appended only. Key ordering is NOT byte-contractual (uses `encoding/json`). |
...
gitmap-v23 startup-list --json | ajv validate -s startup-list.schema.json -d -
```
...
5. **Pair every schema with a contract test** in `gitmap-v23/cmd/` named
