...
| ✅ done | ~~`gitmap-v23 export` (`export.go`)~~ | Migrated to stablejson via `exportrender.go` (v5.81.0). Top-level key order pinned: `version`, `exportedAt`, `repos`, `groups`, `releases`, `history`, `bookmarks`. Nested arrays pre-rendered with `json.MarshalIndent` (per-record key order remains Go-struct-declaration-order, NOT contractual in v1). Empty arrays emit `[]` (never `null`). Schema: [`export.schema.json`](export.schema.json). Pinned by `gitmap/cmd/export_jsonschema_contract_test.go` + `exportjson_contract_test.go`. |
