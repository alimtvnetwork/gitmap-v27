...
`go-winres` embeds icons as Windows `.ico` resources inside the compiled `.exe`. The Windows `.ico` format has a **hard limit of 256x256 pixels** per image frame. The project's `gitmap-v26/assets/icon.png` was **512x512**, which exceeds this limit.

The error originates from the `go-winres make` step in the release workflow (`.github/workflows/release.yml`), which reads `gitmap-v26/winres/winres.json` and converts the referenced PNG into an `.ico` resource.
...
1. Created a 256x256 resized copy: `gitmap-v26/assets/icon-256.png`.
2. Updated `gitmap-v26/winres/winres.json` to reference `icon-256.png` instead of `icon.png`.
...
3. **Add a pre-check step** in CI before `go-winres make`:
   ```bash
   python3 -c "from PIL import Image; img=Image.open('gitmap-v26/assets/icon-256.png'); assert max(img.size)<=256, f'Icon too large: {img.size}'"
   ```
4. **Document the constraint** in `gitmap-v26/winres/README.md` so future contributors don't replace the icon with a higher-resolution version.

### Related Files

| File | Purpose |
|------|---------|
| `gitmap-v26/winres/winres.json` | Windows resource manifest for `go-winres` |
| `gitmap-v26/assets/icon-256.png` | 256x256 icon for `.exe` embedding |
| `gitmap-v26/assets/icon.png` | 512x512 original icon (web/docs) |
...
The compress step ran inside `gitmap-updater/` (which has no `dist/` folder) instead of `gitmap-v26/dist/` where cross-compiled binaries are output. The script used `cd dist` which assumed the working directory was `gitmap-v26/`, but GitHub Actions defaults to the repository root for every `run:` step unless `working-directory` is explicitly set.

In a monorepo with multiple Go modules (`gitmap-v26/`, `gitmap-updater/`), the previous step's working directory does **not** carry over to the next step.

### Why It Wasn't Caught

- Locally, `run.ps1` always executes from `gitmap-v26/`, so `cd dist` works.
