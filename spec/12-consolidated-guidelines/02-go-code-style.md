...
|-------------------|------------------|----------|
| CLI routing / top-level commands | `gitmap-v26/constants/` when shared across dispatcher, completion, tests | `CmdScan`, `CmdReleaseAlias` |
...
1. Use `gitmap-v26/constants/` only for literals shared across packages.
...
`.github/scripts/check-constants-naming.sh` enforces this at a lightweight level by rejecting newly added `gitmap-v26/constants/` names that are not ordinary PascalCase identifiers.
