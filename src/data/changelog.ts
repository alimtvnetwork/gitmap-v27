...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    version: "v5.40.0",
    date: "2026-05-19",
    subtitle: "fix-repo auto-backup + new `gitmap-v26 undo` (alias `ud`) restore command",
    items: [
      "**Auto-backup.** Every `gitmap-v26 fix-repo` write (non-dry-run) now snapshots the pre-rewrite copy of each modified file to `.gitmap/backup/<repo>/v<current>/fix-repo/<UTC-ts>/files/<rel>` alongside a `manifest.json` index. Untouched files are never copied; dry-run never creates a snapshot.",
      "**New command:** `gitmap-v26 undo` (alias `ud`) restores the latest snapshot for the current repo + current version. Subcommands: `--list` (snapshots newest-first, latest marked `*`), `--snapshot <ts>` (pick a specific stamp), `--dry-run` (preview).",
      "**Scoping:** snapshots live under `<repo>/v<current>` so an undo inside `gitmap-v26` can never touch a `gitmap-v26` snapshot. Manifest carries `schemaVersion`, `repo`, `currentVersion`, `timestamp`, `gitmapVersion`, `files[]`.",
  ...
  },
  {
    ...
    items: [
      "**New flag.** `gitmap-v26 fix-repo --restrict no-version` (short: `gitmap-v26 fr -2 -r nv`) suppresses the v1→v2 bare-base rewrite so ONLY `{base}-vN` tokens are touched. Bare `{base}` occurrences are left alone even during a v1→v2 bump.",
      ...
      "`gitmap-v26 help` now lists `--restrict <mode>` under 'Fix-repo flags:' with two copy-pasteable examples (full + short form).",
  ...
  },
  {
    version: "v5.38.0",
    date: "2026-05-19",
    subtitle: "fix-repo bare-base rewrite restricted to v1→v2 (no more corrupting bare `gitmap-v26` at v3+)",
    items: [
      "**Critical fix.** Running `gitmap-v26 fix-repo` inside a `-v3` (or higher) repo no longer rewrites bare `{base}` tokens. Before v5.38.0, `fix-repo` inside `gitmap-v26` would rewrite every standalone mention of `gitmap-v26` (binary names, package identifiers, brand strings, unrelated `https://github.com/owner/gitmap-v26` URLs) to `gitmap-v26` — silently corrupting the working tree.",
  ...
  },
  {
    ...
    items: [
      "Bare `gitmap-v26` and `gitmap-v26 help` now open with a magenta/cyan banner (`🗺  gitmap-v26 vX.Y.Z — Git repo discovery, cloning & release toolkit`) and close with a build-info footer showing the installed version, source repo origin URL, current branch, and the last commit (`shortSHA · subject · relative-date`).",
      ...
      "Reminder for the `gitm` alias: it ships in the shell wrapper installed by `gitmap-v26 setup`. The installer auto-runs setup (v5.18+), but stale installs require a one-time `gitmap-v26 setup` + shell reload to pick up the new function.",
  ...
  },
  {
    version: "v5.36.0",
    date: "2026-05-19",
    subtitle: "PowerShell `gitmap-v26 cd` wrapper: bulletproof `[string]` cast against `Set-Location` Object[] binding",
    items: [
      "Fix: stale-profile users hit `Set-Location : Cannot convert 'System.Object[]' to the type 'System.String' required by parameter 'LiteralPath'` when running `gitmap-v26 cd <slug>` — even after the v5.17.0 `Out-String | Trim` fix — because PowerShell could still bind `$dest` as `Object[]` in some pipelines.",
      "Hardened all four PowerShell wrapper sites: `gitmap-v26/constants/constants_cd.go` (gcd + gitmap-v26 fns), `constants_pathsnippet.go` (setup-written profile snippet), `constants_cd_shim.go` (template), and `gitmap-v26/scripts/install.ps1` (both the one-liner profile wrapper and the multi-line shim template).",
      ...
      "Action required for existing users: re-run `gitmap-v26 setup` (or reinstall) and reload the shell — the marker block `# gitmap-v26 shell wrapper v2` / `# gitmap-v26 command wrapper v1` is rewritten in-place by setup.",
  ...
  },
  {
    version: "v5.31.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v26 pull-release-cd` / `prc` — multi-repo one-shot release",
    items: [
      "Added: `gitmap-v26 pull-release-cd` (alias `prc`) — multi-repo, one-shot pull-release runner. Accepts a comma-separated list of `<name-or-url> <version>` pairs (e.g. `gitm prc gitmap-v26 v5.31.0, marco v2.5.0, https://github.com/me/url-git v3.1.0`) and, for each entry, chdirs into the target repo and spawns `gitmap-v26 pull-release <version> -y` as an isolated subprocess.",
      "Added: URL tokens (containing `://` or starting with `git@`) are cloned first via a `gitmap-v26 clone <url>` subprocess; the derived slug (URL's last path segment minus `.git`) is then resolved against the gitmap-v26 DB for the subsequent pull-release.",
  ...
  },
  {
    version: "v5.30.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v26 push` auto `pull --rebase` + retry on non-fast-forward rejection",
    items: [
      "Added: `gitmap-v26 push` now detects git's non-fast-forward rejection (`[rejected] ... (fetch first)` / `failed to push some refs`), automatically runs `git pull --rebase`, and retries the push once — no more manual `git pull` → `git push` cycle after `gitmap-v26 push -ssh`.",
      "Behavior: stderr is tee'd so users still see git's original rejection message live; the auto-retry path prints `↻ push rejected (non-fast-forward) — auto-running 'git pull --rebase' and retrying`. If rebase hits conflicts, the original git exit code is propagated and a hint to resolve + re-run `gitmap-v26 push` is printed.",
  ...
  },
  {
    version: "v5.29.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v26 push` + `--ssh` / `--https` transport flags on push & pull",
    items: [
      "Added: `gitmap-v26 push` (alias `ph`) — runs `git push` in the current repo with full stdin/stdout/stderr forwarding and exit-code propagation, mirroring the v5.28.0 `gitmap-v26 pull` cwd short-circuit.",
      "Added: `--ssh` / `-ssh` / `--sh` and `--https` / `-https` / `--ht` transport flags on both `gitmap-v26 push` and `gitmap-v26 pull` — rewrites `remote.origin.url` to the requested transport and **persists** the change via `git remote set-url origin`, so subsequent plain `git push` / `git pull` invocations keep the new transport.",
      "Added: when both flags are set, `--ssh` wins with a one-line stderr warning (mirrors `gitmap-v26 clone` semantics from spec 110). Unrecognised origin URLs fail-open — a warning is printed but git push/pull still runs.",
      "Added: extra positional args after the flags forward verbatim, so `gitmap-v26 push --ssh origin main` runs `git push origin main` against the freshly-rewritten SSH origin.",
  ...
  },
  {
    version: "v5.28.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v26 pull` works like `git pull` in a repo + new `gitm` short alias",
    items: [
      "Added: `gitmap-v26 pull` with no args now short-circuits to a plain `git pull` when run inside a git repo — stdin/stdout/stderr forwarded, underlying exit code propagated. Slug / `--group` / `--all` / aliased-repo modes are unchanged.",
      "Added: `gitm` shell alias — Bash / Zsh / PowerShell wrappers installed by `gitmap-v26 setup` now also define `gitm` as a thin forwarder to `gitmap-v26`, so `gitm pull`, `gitm cd <name>`, `gitm clone <url>` all behave identically. Re-run `gitmap-v26 setup` (or reinstall) to pick up the new wrapper block.",
      "Confirmed: `gitmap-v26 setup` auto-run after install has shipped since v5.18.0 — both `install.ps1` and `install.sh` call `gitmap-v26 setup` as a non-fatal final step, so `gitm` is registered automatically on fresh installs.",
  ...
  },
  {
    version: "v5.27.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v26 cfrp` / `cfr` now honour `--ssh` / `--https`",
    items: [
      "Fixed: `gitmap-v26 cfrp <url> --ssh` (and `cfr`) was ignoring the transport flag — `parseCloneFixRepoArgs` only recognised `--no-vscode-sync` and `--require-version`, so the HTTPS URL was passed straight to the in-process clone.",
      "Fixed: URL is now rewritten via `ConvertURLToSSH` / `ConvertURLToHTTPS` before the clone runs, mirroring `gitmap-v26 clone --ssh` exactly.",
      "Added: accepts `--ssh`, `-ssh`, `--sh`, `-sh`, `--https`, `-https`, `--ht`, `-ht` (single- and double-dash, plus the same short aliases as `gitmap-v26 clone`). When both `--ssh` and `--https` are set, `--ssh` wins with a stderr warning.",
  ...
  },
  {
    ...
    items: [
      ...
      "Rolled up: `gitmap-v26 clone <url> --ssh` (and every other bool clone flag) is honoured regardless of position — `parseCloneFlags` routes through `reorderFlagsBeforeArgs` so a trailing `--ssh` no longer slips past Go's `flag` package.",
  ...
  },
  {
    version: "v5.24.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v26 clone --ssh` now works when the flag follows the URL",
    items: [
      "Fixed: `gitmap-v26 clone <url> --ssh` (and `--https`, `--no-replace`, every other bool flag) was silently ignored when written AFTER the positional URL. Go's `flag` package stops parsing at the first non-flag, so the trailing `--ssh` never reached `applyURLSchemeFlags` and the HTTPS URL was cloned as-is.",
      "Fixed: `parseCloneFlags` now routes through `reorderFlagsBeforeArgs` (the same helper used by `release`, `clone-next`, `clone-from`, `commit-transfer`, etc.), so flags are honoured regardless of position. `gitmap-v26 clone --ssh <url>`, `gitmap-v26 clone <url> --ssh`, and `gitmap-v26 clone <url> --ssh --no-replace` all behave identically.",
  ...
  },
  {
    ...
    items: [
      ...
      "Kept: `gitmap-v26/scripts/install.{ps1,sh}` are unchanged source-of-truth (consumed by `go:embed` for `gitmap-v26 self-install`, the `release-version` generators, CI workflows, smoke tests, and ~50 spec/memory docs). Root files are byte-identical copies — same checksum contract, same behavior.",
  ...
  },
  {
    version: "v5.22.0",
    date: "2026-05-18",
    subtitle: "Pin bump — rolls up v5.21.0 `gitmap-v26 ssh view` / `copy` / `create`",
    items: [
      ...
      "No behavior changes since v5.21.0 — this release re-pins the new `gitmap-v26 ssh view` / `copy` / `create` surface (clipboard-aware, soft-fails when no `clip` / `pbcopy` / `wl-copy` / `xclip` / `xsel` is on PATH) as the stable build.",
    ],
  },
  {
    version: "v5.21.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v26 ssh` gets `view` / `copy` / `create` subcommands (clipboard-aware)",
    items: [
      "Added: `gitmap-v26 ssh view <key>` (aliases `v`, existing `cat`) prints the public key to stdout. Same output as `ssh cat` — just a more discoverable verb alongside `create` / `copy` / `list` / `delete`.",
      "Added: `gitmap-v26 ssh copy <key>` (alias `cp`) prints the public key AND pushes it to the OS clipboard in one shot. Picks the right tool per OS: `clip` on Windows, `pbcopy` on macOS, `wl-copy` → `xclip -selection clipboard` → `xsel --clipboard --input` on Linux (first one found). If no clipboard tool is available, the key is still printed and a one-line warning is emitted to stderr — the command never fails.",
      "Added: `gitmap-v26 ssh create <flags>` is an explicit alias for the default `gitmap-v26 ssh` (generate). All existing `--name` / `--path` / `--email` / `--force` flags work unchanged.",
  ...
  },
  {
    version: "v5.20.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v26 clone --ssh` / `--https` coerce every URL into the requested transport before git runs",
    items: [
      "Added: `gitmap-v26 clone --ssh` rewrites every recognised Git URL into its `git@host:owner/repo.git` SSH-shorthand form before `git clone` is invoked. HTTPS (`https://github.com/owner/repo`) and `ssh://git@host[:port]/owner/repo` URLs are both converted; already-shorthand URLs are normalized (`.git` suffix appended). The flag flows through the multi-URL form too — `clone url1,url2,url3 --ssh` converts the whole batch in one shot.",
      "Added: `gitmap-v26 clone --https` is the symmetric counterpart — forces every URL into `https://host/owner/repo.git` form. Useful in CI/headless environments where the SSH agent isn't unlocked. `--ssh` and `--https` are mutually exclusive; when both are set, `--ssh` wins and a one-line stderr warning is printed.",
  ...
  },
  {
    version: "v5.19.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v26 rp` (release-pending) rejects version args + canonical command banner clears `rp` vs `pr` alias confusion",
    items: [
      "Fixed: `gitmap-v26 rp v3.1` no longer silently runs `release-pending` and releases unrelated versions (e.g. an orphaned `v2.233.0` metadata file). The old code parsed `v3.1` into `flag.Args()` and ignored it, so users who meant `pr v3.1` (pull-release) ended up releasing whatever was pending. `runReleasePending` now scans positional args for a version-shaped token (regex `^v?\\d+(\\.\\d+){0,2}(-...)?$`) and exits 2 with a precise suggestion pointing at `gitmap-v26 pr <ver>` or `gitmap-v26 release <ver>`.",
      "Added: canonical command banner at the start of `gitmap-v26 release-pending` and `gitmap-v26 pull-release` — when a short alias (`rp`, `pr`) is invoked, the resolved canonical name + alias print to stderr first (`→ Running: gitmap-v26 release-pending  (alias: rp)`) so it's immediately obvious which pipeline is running. New reusable helper `printCanonicalCmdBanner` in `gitmap-v26/cmd/releasepending.go`.",
  ...
  },
  {
    version: "v5.18.0",
    date: "2026-05-18",
    subtitle: "Auto-run `gitmap-v26 setup` after install and on first `gitmap-v26 cd` when the shell wrapper isn't loaded",
    items: [
      "Install scripts (`gitmap-v26/scripts/install.sh` + `gitmap-v26/scripts/install.ps1`) now invoke `<bin_path> setup` as a final non-fatal step after `verify_installation` / `Invoke-InstallVerification`, so a fresh `irm … | iex` or `curl … | sh` lands the user with shell wrapper (`gcd` / `gitmap-v26` profile function) + completions installed — no second manual command. Setup is idempotent (marker `# gitmap-v26 shell wrapper v2`); failures print a yellow `(setup auto-run skipped …)` line and continue.",
      "`gitmap-v26 cd` self-heals when the shell wrapper isn't loaded: `warnIfNoWrapper` in `gitmap-v26/cmd/setupverify.go` now auto-runs the full `gitmap-v26 setup` (not just `InstallCDFunction`) so the very first `gitmap-v26 cd <repo>` after a fresh install installs the wrapper + completions automatically. Existing stderr reload tip (`. $PROFILE` / `source ~/.zshrc`) still prints so the user knows why the *next* `cd` will actually move the parent shell. Recover-guarded — a setup panic never breaks the cd output.",
      "Closes the first-run UX gap where users hit `! Shell wrapper not active — 'gitmap-v26 cd' printed the path but cannot change your directory.` because `gitmap-v26 setup` was a separate manual step.",
    ],
  },
  {
    version: "v5.17.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v26 cd` PowerShell wrapper: coerce stdout to a single string (fixes `Set-Location : Cannot convert 'System.Object[]'`)",
    items: [
      "`gitmap-v26 cd <repo>` crashed in PowerShell with `Set-Location : Cannot convert 'System.Object[]' to the type 'System.String' required by parameter 'LiteralPath'` whenever the captured stdout parsed as a multi-element array (extra line, CRLF artifact, or auxiliary write turned `$dest` into `System.Object[]`).",
      "All four PowerShell wrapper templates now collapse the captured output with `(& $real … | Out-String).Trim()` so `$dest` is always a single trimmed string before `Test-Path` / `Set-Location`: `gitmap-v26/constants/constants_cd.go` (CDFuncPowerShell `gcd` + `gitmap-v26`), `gitmap-v26/constants/constants_pathsnippet.go` (PathSnippetPwshFmt `Invoke-GitmapAndSetLocation`), `gitmap-v26/constants/constants_cd_shim.go` (gitmap.ps1 shim), `gitmap-v26/scripts/install.ps1` (Get-GitmapCommandWrapperBlock + Get-GitmapPowerShellShimContent).",
      "Users hit by the v5.16.0-and-earlier wrapper must re-run `gitmap-v26 setup` (or re-install) so their `Microsoft.PowerShell_profile.ps1` snippet is rewritten with the fixed body.",
    ],
  },
  {
    version: "v5.16.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v26 release` no longer leaks gitmap-specific content (CHANGELOG body + release-version.{ps1,sh} snapshots) into other repos' releases",
    items: [
      "Release body: `uploadToGitHub` in `gitmap-v26/release/workflowgithub.go` now starts with an empty body and only calls `DetectChangelog()` + `AppendPinnedInstallSnippet` when `ShouldPrintInstallHint(getRemoteURL())` is true. Non-gitmap repos get a tag-only release with an empty body — no more gitmap-v26 CHANGELOG.md notes dumped into unrelated release pages.",
  ...
  },
  {
    ...
    items: [
      "Added a new `--strict` (PowerShell alias `-Strict`) flag to `gitmap-v26 fix-repo` / `fr` and to `fix-repo.ps1`. When supplied, the command runs `go test` against the unique set of Go packages whose `.go` files were modified, AFTER the rewrite step and AFTER the v4.8.0 `gofmt -w` post-step, and exits with the new code **9** (`FixRepoExitTestsFailed`) on any test failure. Off by default — `gitmap-v26 fix-repo` stays a pure rewriter for non-Go repos and for users without a Go toolchain on PATH.",
      "Closes the gap that allowed v4.12.0's bug class to ship in the first place: the rewriter is byte-level and CAN'T detect SEMANTIC desyncs (e.g. a hard-coded sibling literal `\"9\"` next to a `{base}-v9` neighbor that the rewriter bumped to `-v10`). With `--strict`, that class of bug fails the same `gitmap-v26 fix-repo --strict` invocation that would otherwise produce the broken commit, instead of waiting for CI minutes later.",
  ...
  },
  {
    ...
    items: [
      ...
      "On a hit it emits an annotated GitHub error (`::error title=fix-repo gofmt regression::…`) that names the root cause explicitly — \"the v4.8.0+ post-rewrite gofmt step did not run on them\" — followed by the file list, the full `gofmt -d` diff, and three labeled fix recipes: (1) local `gofmt -w .` + amend, (2) re-run `gitmap-v26 fix-repo --all` (auto-formats since v4.8.0), or (3) re-run `pwsh -File fix-repo.ps1 -All` (auto-formats since v4.9.0). This narrows the failure mode from \"some file isn't gofmt-clean, look around\" to \"a fix-repo bump bypassed the auto-format step, here's how to recover\" — readable in a single scroll.",
  ...
  },
  {
    ...
    items: [
      "New end-to-end test `gitmap-v26/tests/fixrepo_test/TestFixRepoGofmtCleanAfterRewrite` builds the gitmap-v26 binary, materializes a throwaway git repo whose origin URL ends in `repo-v9.git` containing two Go files with column-aligned `map[string]string` and `const (...)` blocks straddling the v8/v9/v10 width boundary, runs `gitmap-v26 fix-repo --all --verbose` against it, and asserts both that the output contains a `gofmt:` summary line (proving the v4.8.0 step ran) AND that `gofmt -l .` is silent afterward (proving alignment is preserved).",
  ...
  },
  {
    ...
    items: [
      "`fix-repo.ps1` now mirrors `gitmap-v26 fix-repo`'s v4.8.0 behavior: after the token sweep, every modified `.go` file is collected and passed in a single `gofmt -w @($GoFiles)` invocation, guarded by `Get-Command gofmt -ErrorAction SilentlyContinue` so non-Go environments degrade gracefully with a stderr WARN instead of a hard failure.",
      ...
      "Why this matters for CI: width-crossing version bumps (e.g. `gitmap-v26` → `gitmap-v26`) silently broke column alignment in Go map literals / const blocks because the byte-level token rewriter doesn't understand Go syntax. The Go binary closed this gap in v4.8.0; the `.ps1` helper kept the gap open whenever someone (or a bootstrap script) invoked the script path. Both engines now produce gofmt-clean output by default, so the `cd gitmap-v26 && gofmt -w .` manual step is no longer needed regardless of which entry point you use.",
  ...
  },
  {
    ...
    items: [
      "`gitmap-v26 fix-repo` (alias `fr`) gains a post-rewrite gofmt pass: after the byte-level token sweep finishes, every modified `.go` file is collected and passed in a single batched `gofmt -w <files...>` invocation. This closes the long-standing gap where width-crossing version bumps (e.g. `gitmap-v26` → `gitmap-v26`) silently broke column alignment in Go map literals / const blocks and tripped CI's `gofmt -l .` gate.",
  ...
  },
  {
    ...
    items: [
      "`gitmap-v26 release-pull` (alias `relp`) now accepts three pull modes — `--ff-only` (default, preserves the original safety guarantee), `--rebase` (with auto-abort on conflict so the working tree is never left mid-rebase), and `--merge` (creates a merge commit when fast-forward isn't possible). Modes are mutually exclusive; passing more than one fails fast with a precise error listing the conflicting flags. All other args after the mode flag are forwarded verbatim to `gitmap-v26 release`, including `--dry-run` and `--verbose`.",
      "Top-level `gitmap-v26 help` now renders a dedicated **Fix-repo flags** section listing `-2` (default), `-3`, `-5`, `--all`, `--dry-run`, `--verbose`, `--config`, plus the full exit-code legend (`0 ok | 2 not-a-repo | 3 no-remote | 4 no-vN-suffix | 5 bad-version | 6 bad-flag | 7 write-failed | 8 bad-config`). New constants live in `gitmap-v26/constants/constants_fixrepohelp.go`; rendering is wired through `printUsageFixRepoFlags()` in `gitmap-v26/cmd/rootusageflags.go`.",
  ...
  },
  {
    version: "v3.119.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v26 inject` / `inj`: register an existing folder with Desktop + VS Code (+ DB)",
    items: [
      "New command for the 'I already have this repo on disk, just plug it into my tooling' workflow — no clone, no scan, just register. Forms: `gitmap-v26 inject` (cwd), `gitmap-v26 inject <folder>` (absolute, relative, or ~-prefixed), `gitmap-v26 inj <folder>` (short alias).",
  ...
  },
  {
    version: "v3.118.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v26 clone <url>` cds into the cloned folder",
    items: [
      "Single-URL `gitmap-v26 clone <url>` now writes the cloned folder's absolute path to the shell-handoff sentinel, so the wrapper function chdirs the parent shell into the new repo — same UX as `gitmap-v26 cn` and `gitmap-v26 cd`.",
  ...
  },
  {
    version: "v3.117.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v26 cn vX <folder>` + `gitmap-v26 cn <folder>` (defaults to v++); hero card UI polish",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      "Generator-aware skip markers on the full subcommand strings: `CmdTemplatesDiff` and `CmdTemplatesInit` carry `// gitmap-v26:cmd skip` line comments. `\"diff\"` would be a no-op (already a top-level command — folder-tree diff via `gitmap-v26/cmd/diff.go`); `\"init\"` would mislead users into typing `gitmap-v26 init` standalone. Both decisions are now documented in source instead of in a side-channel memory note.",
  ...
  },
  {
    ...
    items: [
      ...
      "UX deviations from original plan: positional `<lang>...` chosen over `--lang <csv>` (more natural for a scaffolder), and `--cwd` deferred (`cd && gitmap-v26 templates init …` covers it). Both deviations recorded in `.lovable/memory/plans/05-templates-polish-plan.md` Phase 2 notes.",
  ...
  },
  {
    ...
    items: [
      ...
      "Plan 05 status: Phase 0 (spec) ✅, Phase 1 (langs) ✅, Phase 3 (`templates diff`) ✅. Remaining: Phase 2 (`gitmap-v26 templates init` scaffolder), Phase 4 README snippet (deferred until `init` lands), Phase 5 (QA + tag).",
  ...
  },
  {
    version: "v3.108.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v26 templates diff` (alias `td`) — preview template changes without writing",
    items: [
      "Added `gitmap-v26 templates diff` (alias `td`) — pure, write-nothing preview of what `gitmap-v26 add ignore <lang>` / `gitmap-v26 add attributes <lang>` would change. Marker-block aware: only the region between the gitmap-managed `BEGIN`/`END` markers is compared, so edits outside the block never trigger a false positive.",
      "Status enum drives behavior and exit codes (mirrors `diff(1)`): `DiffNoChange` → exit 0, `DiffMissingFile` / `DiffMissingBlock` / `DiffBlockChanged` → exit 1, hard errors (bad lang, unreadable tree) → exit 2. This makes the command pre-commit-hook safe: `gitmap-v26 templates diff --lang go || gitmap-v26 add ignore go`.",
  ...
  },
  {
    ...
    items: [
      "Fixed: root cause of the recurring `pending task already exists ... \\https:\\github.com\\...` + `fatal: could not create leading directories` failure for `gitmap-v26 clone <url1> <url2>` is that the user's deployed `gitmap.exe` on PATH is older than v3.80.0 and does not contain the multi-URL routing fix. Current source already routes 2+ URLs to `runCloneMulti` via `shouldUseMultiClone(cf)` — but the stale binary still reaches `executeDirectClone(url, folderName=<second URL>, ...)`, builds the impossible Windows path `D:\\...\\https:\\github.com\\...`, and crashes git.",
      "Fixed: `gitmap-v26/cmd/clone.go` `executeDirectClone` now refuses URL-shaped folder names early with an actionable message that names the exact recovery commands: `gitmap-v26 doctor`, `gitmap-v26 update`, `gitmap-v26 pending clear --yes`, and the reminder to open a NEW terminal so PATH refreshes. This shape is impossible in current source, so the guard fires only when a stale binary is in use — and the message tells the user exactly that, instead of letting git fail with `Invalid argument`.",
      ...
      "Action required: the source is correct, but your PATH binary must be replaced. Run `gitmap-v26 doctor` to confirm the active version, `gitmap-v26 update` to redeploy, open a NEW terminal so PATH refreshes, then `gitmap-v26 pending clear --yes` to drop the orphaned row, and retry the clone — it works space- or comma-separated on PowerShell and bash.",
    ],
  },
  {
    version: "v3.94.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Added: `gitmap-v26/cmd/root_url_shortcut_test.go` pins the bare-URL shortcut against the three exact `Unknown command` invocations users keep reporting — `gitmap-v26 https://a,https://b,https://c` (PowerShell paste), `gitmap-v26 https://a, https://b, https://c` (bash paste, comma-then-space), and `gitmap-v26 https://a, https://b https://c` (mixed separators). Also covers single-URL, leading-flag (`--verbose <url>`), SSH-shorthand (`git@github.com:a/b.git`), and GitLab variants, plus negative cases (known subcommand, folder path, empty argv) so the shortcut never grabs a legitimate command. Three test functions, all sub-second, no external deps.",
      ...
      "Diagnostic note for users still hitting `Unknown command: https://...`: the shortcut has been in source since before v3.91.0, but a stale deployed binary on `PATH` will not have it. Run `gitmap-v26 doctor` to confirm the active binary version, then `gitmap-v26 update` to deploy the current source. The startup version-check banner from v3.90.0 also surfaces this gap on every invocation.",
    ],
  },
  {
    version: "v3.57.0",
    date: "2026-04-23",
    subtitle: "Archive system: `gitmap-v26 zip` + `gitmap-v26 unzip-compact` (alias `uzc`) with compact-folder normalization, multi-source resolution (local / HTTP / git), and `ArchiveHistory` SQLite persistence",
    items: [
      "Added `gitmap-v26 unzip-compact` (alias `uzc`) — extracts a local archive, an HTTP(S) URL, or auto-picks the single archive in the current folder, into a single normalized output folder under `<dest>/<archive-base-name>/`. The compact-extract algorithm (`gitmap-v26/archive/extract.go`) uses the temp-dir-then-move strategy: stream-extract into a sibling `.gitmap-uzc-*` temp dir, walk to find the deepest single-child directory chain (capped at `MaxCompactFlattenLayers=4` per the spec's 2..4 tolerance), then promote that real root into the final folder. Result: `xap.zip → xap/xap/<files>` collapses to `dest/xap/<files>`, `xlt.zip → <files>` (no folder) wraps cleanly into `dest/xlt/<files>`, and `mixed.zip → README + src/` lands as `dest/mixed/{README,src}` (no flatten possible).",
      "Added `gitmap-v26 zip` — accepts N heterogeneous sources on the command line (folders, HTTP(S) archive URLs, git URLs) and writes one archive whose format is derived from the `--out` extension. HTTP sources are downloaded into a temp dir using `aria2c` when available, falling back to `net/http` otherwise; git sources are shallow-cloned (`git clone --depth=1`) into a temp dir; both temp workspaces are removed via `defer cleanupAllSources` regardless of success or failure. Each source root maps to `<basename>/` in the archive so multi-source bundles stay tidy.",
      ...
      "Listing mode: `gitmap-v26 uzc --list <archive>` (also `-l`) prints the entry table to stderr without extracting, capped at 50,000 entries to keep malicious archives from exhausting memory. Auto-detect mode: bare `gitmap-v26 uzc` (no positional args) scans the current folder via `AutoDetectSingleArchive` and proceeds only when EXACTLY one archive is found, erroring with the explicit count otherwise (`current folder has N archives — pass one explicitly`).",
      ...
      "Wired into `gitmap-v26/cmd/roottooling.go` via the existing dispatch table (no new branch in `dispatch()`). Reused `cmd_constants_parity_test.go`'s registry — added `CmdUnzipCompact`, `CmdUnzipCompactAlias`, and `CmdZip` to `topLevelCmds()`. Resolved a name collision: `z` was already taken by `zip-group`, so the archive `zip` command ships without a single-letter alias (users wanting one can `gitmap-v26 alias add`).",
      ...
      "Bumped `constants.Version` to `3.57.0`. The cmd layer prints `▶ gitmap-v26 <cmd> v<X.Y.Z>` at the top of every archive run so the version that produced any subsequent output / history row is unambiguous.",
    ],
  },
  {
    version: "v3.56.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Storage reuses the existing `Setting(Key TEXT PK, Value TEXT)` table — no schema bump, no `SettingTypes` table. The full document is stored as a JSON blob under `Setting[DownloaderConfig]`, the seed file's hash under `Setting[DownloaderConfigSeedHash]`, and the gitmap-v26 version that last touched the DB under `Setting[DatabaseVersion]`. The `SettingType` enum (`DownloaderConfig` / `DatabaseVersion` / `SystemConfig`) lives in code in `gitmap-v26/constants/constants_downloader.go` so future tooling can route by category without parsing the JSON value.",
  ...
  },
  {
    ...
    items: [
      ...
      "Implemented `--force` as a pre-merge `os.Remove(target)` (ignoring `ErrNotExist`), so `templates.Merge` then takes the `MergeCreated` path and writes a clean gitmap-v26 block. WITHOUT `--force`, hand edits OUTSIDE the marker block always survive (Merge appends a new block alongside) — locked in by `TestExecuteTemplatesInitWithoutForcePreservesUserContent`. WITH `--force`, pre-existing user content is discarded — locked in by `TestExecuteTemplatesInitForceReplacesExisting`. The asymmetry between the two tests is the contract: `--force` is the explicit opt-in for destructive overwrites.",
  ...
  },
  {
    ...
    items: [
      ...
      "Refactored `gitmap-v26/helptext/print.go` to add `PrintWithMode(command, mode)` as the new preferred entry point; `Print(command)` is now a thin `PrintWithMode(command, render.PrettyAuto)` wrapper. Wired both `gitmap-v26/cmd/helpcheck.go::checkHelp` (every `<cmd> --help`) and the `gitmap-v26 help <cmd>` dispatcher in `rootutility.go` to call `ParsePrettyFlag` first and forward the mode. Net effect: every command in the CLI now accepts `--pretty` / `--no-pretty` on its `--help` invocation without per-command code changes.",
  ...
  },
  {
    version: "v3.23.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v26 templates show` optionally pretty-renders markdown templates on a TTY (`--raw` + `GITMAP_NO_PRETTY` opt-outs)",
    items: [
  ...
  },
  {
    version: "v3.20.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v26 add lfs-install` — `git lfs install --local` + idempotent marker-block merge of the `lfs/common` template",
    items: [
  ...
  },
  {
    version: "v3.19.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v26 help scan` — four end-to-end workflows for `--config` / `--mode` / `--output`",
    items: [
      "Added an **End-to-End Examples** section to `gitmap-v26/helptext/scan.md` with four complete workflows that string the three most-asked-about flags together: (E2E 1) custom config + JSON output + re-clone on another host via `gitmap-v26 clone <file> --target-dir`, exercising hierarchy preservation across machines; (E2E 2) one scan, two consumers — SSH-mode JSON for CI plus CSV for spreadsheets, with a custom `--output-path`; (E2E 3) terminal preview → author config → re-scan with `--output json`, the recommended onboarding flow when you don't yet know what to exclude; (E2E 4) CLI flag overrides config file, demonstrating `config.MergeWithFlags` precedence without mutating the file on disk.",
      "Each example is copy-pasteable: shell-prompt-free commands, real artifact paths under `.gitmap/output/`, and an **Expected output** block where the result isn't obvious. Cross-references to `gitmap-v26 clone` make the scan→clone round-trip discoverable from the scan page alone — users no longer need to read two helptext files to understand the full pipeline.",
  ...
  },
  {
    version: "v3.18.0",
    date: "2026-04-22",
    subtitle: "Pretty renderer wired into `gitmap-v26 help` — TTY-aware, opt-out via `GITMAP_NO_PRETTY`",
    items: [
      "`gitmap-v26 help <cmd>` and `<cmd> --help` / `-h` now route the embedded markdown through `render.RenderANSI` when stdout is a TTY: redundant fenced blocks collapse to yellow `→` lines, `\"double-quoted\"` strings render in cyan, italic subtitles directly under a heading become muted, and bodies under headings get a 2-space indent. Existing call sites in `gitmap-v26/cmd/helpcheck.go` and `gitmap-v26/cmd/rootutility.go` get the new behavior automatically — no flag needed, no per-command edits.",
      "Pipes and redirects keep the raw markdown bytes unchanged: when stdout is not a character device (e.g. `gitmap-v26 help scan | less`, `gitmap-v26 help scan > scan.md`, CI capture), the file is printed verbatim so editors / pagers / file diffs aren't polluted with ANSI escape sequences. TTY detection uses the same dependency-free `os.Stat() & ModeCharDevice` check that `gitmap-v26/cmd/scanprogress.go` uses for stderr — no `golang.org/x/term` import added.",
      "Added an opt-out env var `GITMAP_NO_PRETTY=1`: when set to any non-empty value, the renderer is bypassed and raw markdown is printed even on a TTY. Lets users who prefer plain output (or who hit terminal-emulator quirks) disable the pipeline without losing other gitmap-v26 coloring (status glyphs, scan progress, changelog renderer all stay on).",
  ...
  },
  {
    version: "v3.16.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v26 templates list` / `templates show` — discover the embedded corpus and your overlay",
    items: [
      "Added `gitmap-v26 templates` subcommand router (alias `tpl`) in `gitmap-v26/cmd/templatescli.go`, wired into `dispatch()` in `root.go` (alongside the previously-missed `dispatchAdd` hook, now also re-confirmed). Subcommands: `list` (alias `tl`) and `show <kind> <lang>` (alias `ts`).",
      "Added `gitmap-v26 templates list` — prints a `KIND / LANG / SOURCE / PATH` table with stable kind ordering (ignore → attributes → lfs) then alphabetical lang. The `SOURCE` column reads `user` when the template comes from `~/.gitmap/templates/` and `embed` when it comes from the binary, making it obvious which entries you've customized.",
      "Added `gitmap-v26 templates show <kind> <lang>` — writes the resolved template (overlay > embed) to stdout *with* its audit-trail header preserved, so `gitmap-v26 templates show ignore go > .gitignore.curated` round-trips cleanly and is diffable against future re-curations.",
  ...
  },
  {
    version: "v3.15.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v26 add attributes` + `gitmap-v26 add lfs-install` — completes the templates trio",
    items: [
      "Added `gitmap-v26 add attributes [langs...]` (alias `aa`) in `gitmap-v26/cmd/addattributes.go` — mirror of `add ignore` but writes to `./.gitattributes` using the dedicated `# >>> gitmap-attributes (do not edit between markers) >>>` / `# <<< gitmap-attributes <<<` markers. Same single-pass dedupe, atomic write, and `Changed=false` no-op on re-run.",
      "Added `gitmap-v26 add lfs-install` (alias `alfs`) — runs `git lfs install --local` (streaming stdout/stderr so users see the LFS hook installation) then merges the curated `lfs/common.gitattributes` into `./.gitattributes`. The LFS template uses the attributes markers so it co-exists cleanly with `add attributes`.",
  ...
  },
  {
    version: "v3.14.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v26 add ignore` — curated, idempotent .gitignore templates with marker-block merge",
    items: [
      "Added `gitmap-v26 add` subcommand router (`gitmap-v26/cmd/rootadd.go`, `gitmap-v26/constants/constants_add.go`) wired into `dispatch()` in `root.go`. Top-level alias `ad`. Routes to `add ignore` (alias `ai`), `add attributes` (alias `aa`, Phase 3), and `add lfs-install` (alias `alfs`, Phase 3).",
      "Added `gitmap-v26 add ignore [langs...]` — composes `common` plus each requested language template into a marker-block (`# >>> gitmap-ignore ... >>>` / `# <<< gitmap-ignore <<<`), preserves user-authored entries below `# user entries`, and writes atomically via temp file + rename.",
  ...
  },
  {
    ...
    items: [
      ...
      "Renamed `gitmap-v26` → `gitmap-v26` across Go constants, install/uninstall scripts, spec docs, helptext, README, React data files, and GitHub workflows. Only `.gitmap/` artifacts retain v3 references (immutable per project policy).",
  ...
  },
  {
    ...
    items: [
      ...
      "Added parallel `gitmap-v26 pull` worker pool (`pullparallel.go`) with a mutex-guarded `BatchProgress` tracker, opt-in via `--parallel <N>`.",
      "Added `--only-available` pull pre-filter (`pullfilter.go`) that intersects the target repo list with `FindNext` results so `gitmap-v26 pull --only-available` skips repos with no new tags. Fail-open if the database is inaccessible.",
      "Added `gitmap-v26 help probe` and `gitmap-v26 help sf` docs (synopsis, flags, examples, 3–8 line realistic terminal simulation) in `gitmap-v26/helptext/`.",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "`gitmap-v26 update-cleanup` now scans the active PATH directory, the PATH-derived deploy directory, the configured deploy directory, and the repo build output directory so stale `.old` backups are removed even when `powershell.json` points to an older location.",
      "`gitmap-v26 update-cleanup` now removes leftover `gitmap-update-*` artifacts from deploy/build locations in addition to `%TEMP%`, preventing handoff files from being left behind after update flows that switch between deploy targets.",
    ],
  },
  {
    version: "v2.82.0",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "Hidden `set-source-repo` command — persists source repo path to DB so `gitmap-v26 update` always uses the correct location after repo moves.",
  ...
  },
  {
    ...
    items: [
      ...
      "Installer banner now displays version number (`gitmap-v26 installer v1.0.0`).",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "New `gitmap-v26 version-history` (`vh`) command: displays all version transitions for the current repo with `--limit N` and `--json` support.",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "`gitmap-v26 clone-next` now flattens by default: clones into the base name folder (no version suffix). `gitmap-v26 cn v++` inside `macro-ahk-v15` clones into `macro-ahk/`.",
      "`gitmap-v26 clone <url>` auto-flattens versioned URLs when no custom folder is given (e.g., `wp-onboarding-v13` clones into `wp-onboarding/`).",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "`gitmap-v26 doctor setup` checks: verifies `git-setup.json` resolution from the installed binary location and warns when the shell wrapper is not loaded.",
      "Shell wrapper scripts now export `GITMAP_WRAPPER=1` to detect raw binary vs. wrapper invocation at runtime.",
      "`gitmap-v26 setup` resolves `git-setup.json` relative to the binary path instead of CWD, fixing 'file not found' errors when run from other directories.",
      "Post-setup verification step checks whether the shell function is loaded and prints reload instructions if not.",
      "`gitmap-v26 cd` prints a stderr warning when called without the shell wrapper, guiding users to run `gitmap-v26 setup` or reload their profile.",
    ],
  },
  {
    version: "v2.72.0",
  ...
  },
  {
    ...
    items: [
      ...
      "`gitmap-v26 update` PATH sync now includes full 3-step fallback: direct copy, rename-then-copy, and kill stale processes.",
  ...
  },
  {
    ...
    date: "2026-04-08",
    items: [
      "`run.ps1` and `run.sh` now auto-detect the globally installed `gitmap-v26` location and deploy there instead of using a hardcoded path.",
      ...
      "Replaced hardcoded `E:\\bin-run` in `gitmap-v26 doctor` fix suggestion with dynamic guidance.",
    ],
  },
  {
    version: "v2.66.0",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Added `gitmap-v26 install scripts` — clones gitmap-v26 scripts to a local folder (Windows: D:\\ drive, Linux: ~/Desktop/gitmap-scripts).",
    ],
  },
  {
    version: "v2.63.0",
    date: "2026-04-07",
    items: [
      "Added `installed-dir` (alias `id`) command — prints the active gitmap-v26 binary path and directory.",
      ...
      "Fixed `gitmap-v26 install` on Ubuntu: runs `apt-get update` before install; added `-y` flag and error logging to `.gitmap/logs/`.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Auto-detect pending release branch — running `gitmap-v26 r` on a `release/*` branch with no tag now completes the release instead of erroring.",
      "Running `gitmap-v26 release v1.1.0` while on `release/v1.1.0` delegates to the pending release flow automatically.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Post-release install hints — after releasing a gitmap-v26 repo, the CLI prints Windows (PowerShell) and Unix (Bash) install one-liners.",
      "Repo detection via `GitmapRepoPrefix` constant — only gitmap-v26 repos trigger the install hint output.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Fixed `-y` / `--yes` flag silently ignored when placed after the version argument (e.g., `gitmap-v26 release v2.55 -y`).",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Clone-on-missing-path for `gitmap-v26 update` — when the user provides a non-existent path at the interactive prompt, the system clones the source repo there instead of rejecting it.",
  ...
  },
  {
    ...
    items: [
      ...
      "Auto-confirm skips the commit prompt — enables `gitmap-v26 release v2.55.0 -y` without user input.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "`gitmap-v26 update` now validates the saved source repo path exists on disk before using it; falls back to SQLite DB.",
  ...
  },
  {
    version: "v2.53.0",
    items: [
      "New `gitmap-v26 help-dashboard` (alias `hd`) command to serve the documentation site locally.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added `gitmap-v26 install` system with SQLite-backed `InstalledTools` table tracking name, version, manager, and timestamps.",
      "Added `gitmap-v26 uninstall` command with `--dry-run`, `--force`, and `--purge` flags.",
      ...
      "Reorganized `gitmap-v26 help` with `--compact` flag and 17 categorized command groups.",
      "Repository renamed from `git-repo-navigator` to `gitmap-v26`.",
    ],
  },
  {
    version: "v2.51.0",
    items: [
      "Reorganized `gitmap-v26 help` output into 17 categorized command groups (Scanning, Cloning, Git Operations, Navigation, Release, etc.).",
      "Added `--compact` flag to `gitmap-v26 help` for a minimal command-and-alias-only listing.",
      "`gitmap-v26 help --compact <group>` filters compact output by group name (case-insensitive, falls back to all on no match).",
  ...
  },
  {
    ...
    items: [
      ...
      "`gitmap-v26 ls` now prints resolved DB path when `--verbose` or zero repos found.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added interactive TUI mode (`gitmap-v26 interactive` / `gitmap-v26 i`) built with Bubble Tea.",
  ...
  },
  {
    ...
    items: [
      ...
      "Improved `gitmap-v26 ls <type>` output with labeled fields and inline `cd` examples.",
      "Added shell completion for `release`, `release-branch`, `group`, `multi-group`, and `list` commands.",
      "Fixed duplicate hints after `gitmap-v26 ls <type>` output.",
    ],
  },
  {
    version: "v2.13.0",
    items: [
      "Added group activation: `gitmap-v26 g <name>` sets a persistent active group for batch pull/status/exec.",
      "Added `multi-group` (mg) command for selecting and operating on multiple groups at once.",
      "Added `gitmap-v26 ls <type>` filtering: `gitmap-v26 ls go`, `gitmap-v26 ls node`, `gitmap-v26 ls groups`.",
      ...
      "`gitmap-v26 list-versions` now shows changelog notes.",
    ],
  },
  {
    version: "v2.12.0",
    date: "2026-03-14",
    items: [
      "Added `gitmap-v26 list-versions` (`lv`) command: show all release tags sorted highest-first with changelog.",
      "Added `gitmap-v26 revert <version>` command: checkout tag and handoff rebuild.",
  ...
  },
  {
    ...
    date: "2026-03-06",
    items: [
      "Added `gitmap-v26 cd` (`go`) command: jump to any tracked repo by slug or partial name.",
      "Added `gitmap-v26 watch` (`w`) command: live terminal dashboard monitoring repo status.",
      "Added `gitmap-v26 diff-profiles` (`dp`) command: compare two profiles side-by-side.",
      "Added clone progress bars with retry logic and Windows long-path warnings.",
      "Added `gitmap-v26 gomod` (`gm`) command: rename Go module path across an entire repo.",
  ...
  },
  {
    ...
    date: "2026-03-06",
    items: [
      "Added `gitmap-v26 profile` (`pf`) command: manage multiple database profiles.",
      "Added `gitmap-v26 export` (`ex`) command: export the full database as portable JSON.",
      "Added `gitmap-v26 import` (`im`) command: restore database from a backup file.",
      "Added `gitmap-v26 bookmark` (`bk`) command: save and replay command+flag combinations.",
      "Added `gitmap-v26 stats` (`ss`) command: aggregated usage statistics from command history.",
      "Added `gitmap-v26 history` (`hi`) command: queryable audit trail of CLI executions.",
      "Added `gitmap-v26 amend` (`am`) command: rewrite author name/email on existing commits.",
      "Added `gitmap-v26 seo-write` (`sw`) command: automated SEO commit scheduler.",
    ],
  },
  {
    version: "v2.4.0",
    date: "2026-03-06",
    items: [
      "Added `gitmap-v26 latest-branch` (`lb`) command.",
      "Positional integer shorthand: `gitmap-v26 lb 3` equals `gitmap-v26 lb --top 3`.",
  ...
  },
  {
    ...
    date: "2026-03-06",
    items: [
      "Added `gitmap-v26 list-releases` (`lr`) command.",
      "Enhanced `gitmap-v26 scan` to import `.gitmap/release/v*.json` metadata files.",
