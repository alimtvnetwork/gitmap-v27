...
const directJumpDemo = [
  { text: "gitmap-v23 cd myrepo", type: "input" as const, delay: 800 },
...
const fuzzyDemo = [
  { text: "gitmap-v23 cd apii", type: "input" as const, delay: 800 },
  { text: "", type: "output" as const },
...
const pickerDemo = [
  { text: "gitmap-v23 cd repos", type: "input" as const, delay: 800 },
  { text: "", type: "output" as const },
...
Alias:    go
Usage:    gitmap-v23 cd <repo-name|repos> [flags]
...
Prerequisites:
  ...
  • Run 'gitmap-v23 setup' to install the gcd shell wrapper

Examples:
  gitmap-v23 cd myrepo              Jump to repo directory
  gitmap-v23 cd repos               Interactive repo picker
  gitmap-v23 cd repos --group work  Pick from work group only
...
const CdPage = () => (
  <DocsLayout>
    ...
    <CodeBlock code={helpText} language="bash" title="gitmap-v23 cd --help" />
    ...
    <TerminalDemo title="gitmap-v23 cd — direct jump" lines={directJumpDemo} autoPlay />

    <h2 className="text-xl font-heading font-semibold mt-10 mb-3 docs-h2">Fuzzy Matching</h2>
    <p className="text-sm text-muted-foreground mb-3">
      When no exact match is found, gitmap-v23 suggests the closest repos ranked by Levenshtein distance:
    </p>
    <TerminalDemo title="gitmap-v23 cd — fuzzy suggestion" lines={fuzzyDemo} />
    ...
    <TerminalDemo title="gitmap-v23 cd repos — interactive picker" lines={pickerDemo} />
    ...
    <p className="text-sm text-muted-foreground mb-3">
      The <code className="docs-inline-code">gcd</code> function is auto-installed by <code className="docs-inline-code">gitmap-v23 setup</code> into
  ...
  if [ -n "$dir" ] && [ -d "$dir" ]; then
    ...
    <CodeBlock code="gitmap-v23 cd myrepo" title="Jump to repo by name" />
    <CodeBlock code="gitmap-v23 go myrepo" title="Using alias 'go'" />
    <CodeBlock code="gitmap-v23 cd repos" title="Interactive picker" />
    <CodeBlock code="gitmap-v23 cd repos --group backend" title="Pick from group" />
    <CodeBlock code="gcd myrepo" title="Shell wrapper" />
    <CodeBlock code="gitmap-v23 cd -A api" title="Navigate via alias" />
