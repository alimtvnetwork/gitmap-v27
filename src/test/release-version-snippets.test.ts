  ...
  it("matches snapshot", () => {
    expect(snip).toMatchInlineSnapshot(`
      ...
      & $env:TEMP\\rv.ps1 -Version v3.39.0",
        "pinned": "# Pinned install — locks gitmap-v23 to v3.39.0 (no auto-upgrade)
      iwr https://github.com/alimtvnetwork/gitmap-v23/releases/download/v3.39.0/release-version-v3.39.0.ps1 -OutFile $env:TEMP\\rv.ps1
      & $env:TEMP\\rv.ps1",
      }
    `);
  });
  ...
  it("matches snapshot", () => {
    expect(snip).toMatchInlineSnapshot(`
      ...
      curl -fsSL https://gitmap.dev/scripts/release-version.sh | bash -s -- --version v3.39.0",
        "pinned": "# Pinned install — locks gitmap-v23 to v3.39.0 (no auto-upgrade)
      curl -fsSL https://github.com/alimtvnetwork/gitmap-v23/releases/download/v3.39.0/release-version-v3.39.0.sh | bash",
      }
    `);
  });
});
