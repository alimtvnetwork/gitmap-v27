...
Script-only fallback for the Go-native `gitmap-v27 make-public` /
`gitmap-v27 make-private` commands. Use these when you can't ship the
`gitmap-v27` binary (CI runners, ad-hoc remotes, etc.).
...
These match the exit codes emitted by `gitmap-v27 make-public` /
`gitmap-v27 make-private` so wrappers can branch on the same numbers.
